...
- [5. Definitions of Done](#5-definitions-of-done)
  ...
  - [Product Rendering Through Slots](#product-rendering-through-slots)
...
| `cpt-frontx-calendar-kit-actor-host-application` | Owns data, solution-version state, permissions and backend operations; confirms or cancels move requests. |
...
- The host maps its events into the neutral UI model and renders the week grid; every user action returns to the host as a typed callback, and product-specific content renders through slots.
...
7. [ ] - `p1` - Product-specific card, conflict and detail content renders through `renderEvent`, `renderConflict` and `renderDetail`; conflicts show their dimension plus the host-supplied label - `inst-wg-slots`
...
The system **MUST** receive all events, the active date, the interaction mode and permission/read-only state from the host as props over the neutral UI model, and **MUST NOT** import or call any Schedule backend, solver, planning service, Redux slice or i18n registry; solution-version switching stays host state.
...
### Product Rendering Through Slots

- [ ] `p1` - **ID**: `cpt-frontx-calendar-kit-dod-week-grid-slots`

The system **MUST** render event cards, conflicts and the detail panel through the host's `renderEvent`, `renderConflict` and `renderDetail` slots with their typed contexts, **MUST** display each conflict's dimension plus the host-supplied label without inventing product vocabulary, and **MUST NOT** render loading, empty, or error-state UI.
