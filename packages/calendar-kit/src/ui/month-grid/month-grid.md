...
Use `renderEvent`, `renderCell`, and `renderOverflow` for product-specific presentation without importing application or API types. Styling uses CSS Modules and `--cal-*` theme aliases; consumers can add `className` or override the calendar theme contract without changing component source.
