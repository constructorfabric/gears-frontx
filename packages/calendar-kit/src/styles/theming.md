...
## Option 2 — study the optional OneCloud example preset
...
`themes/onecloud.css` is an optional example preset that shows how to map every `--cal-*` seam; it is not the recommended or default look. It defines every seam with OneCloud design-system values (light and dark) as literals. It stands alone — it does not require your own token sheet to be loaded, and load order relative to `theme.css` does not matter, because custom properties cascade independently of the rule that consumes them.
