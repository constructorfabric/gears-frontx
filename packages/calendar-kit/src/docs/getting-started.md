...
`theme.css` defines the calendar's design tokens. On its own it renders a neutral default palette; map the seams to your own design tokens (see [Theming](../styles/theming.md)) or study the optional `themes/onecloud.css` example preset for one complete mapping. Component CSS arrives automatically with each component import.
