...
Only `theme.css` is loaded, so every token uses its neutral default. Map the seams to your design tokens (see [Theming](../styles/theming.md)); the optional `themes/onecloud.css` example preset shows one complete mapping.
