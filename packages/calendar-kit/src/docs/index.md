...
`@gears-frontx/calendar-kit` is the calendar UI shared by Constructor products: week, day, month and agenda views, the event card and detail, the create-event form, and the side-panel tools around them. It renders what the host gives it and reports what the user did. It never fetches, stores or decides anything about scheduling.
