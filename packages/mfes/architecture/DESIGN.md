...
#### MFES-8 — Origin-invariant chain execution and acceptance-only dispatch

- [ ] `p2` - **ID**: `cpt-frontx-constraint-mfes-origin-invariant-chain-execution`

A chain's execution is origin-invariant. The branch sequence it selects, and the settlement class each of its actions reaches, depend only on the chain as declared and on the reachable topology and handler behaviour at execution time. They also depend on each node's own attempt measured against its own bound, and on nothing else. None of that depends on which runtime, nesting depth, or lifecycle stage dispatched the chain. Dispatching the same chain from the shell, from a mid-tree registry, or from a leaf produces the same handler-and-fallback sequence and the same per-node settlement. That holds because resolution, per-node timeout enforcement, and branch selection are executor-internal decisions: the origin can trigger them but cannot see or influence them. A chain is an independent execution, owned at each step by whichever runtime is executing its current action, and running alongside whatever triggered it. It advances one action at a time. The runtime that executes an action selects `next` or `fallback` from that action's own boolean outcome alone, and dispatches the selected continuation itself. There is no third outcome, and nobody waits on the dispatch. A chain is in that sense history-agnostic, and origin invariance is the shape that takes for origins: nothing outside the current action's outcome selects a branch, and the origin is one such thing. Independence runs in both directions. A chain lives in whichever runtime is executing its current action, bounded by that runtime and never by the life of whatever emitted it. So an extension being unmounted, destroyed, or permanently unregistered leaves the chains it emitted running under whichever runtime is executing them. Teardown of that runtime's executor while it holds the chain is the boundary that ends it, never an outcome of the action the chain was running. A runtime that handed the chain across a hop holds nothing for it afterwards, so its own teardown ends nothing.

The guarantee is conditional, not absolute. It holds for origins within one connected routing tree, once registration propagation between those origins has converged, and absent concurrent mutation of the routing topology while the chain is executing. The first two qualifications concern reachability, not how an already-reached node executes. A registry constructed outside the synchronous mount window that would have let it adopt an inbound bridge behaves, correctly, as a root holding no such bridge. There is no sound way to attribute a late-constructed registry to the extension whose mount produced it. So a target reachable from an ancestor of that extension is genuinely unreachable from the detached registry, not merely slower to reach. Claiming origin invariance without this qualification would assert a reachability guarantee the topology itself does not provide. The third qualification rests on a different fact. Each node resolves against the routing topology as it stands the moment it executes. When that moment falls relative to a concurrent mutation depends on timing that differs by trigger site, nesting depth, and hop count. A retraction landing mid-chain can therefore change what a later node reaches. That is a property of the topology's own timing, not of the origin that triggered the chain, which is why it stays outside the guarantee rather than being folded into it.

"Outcome" is scoped to the executor's logical result: which branch a chain selects and which settlement class each action reaches, not identical eventual side effects. A handler that times out is not cancelled and may keep running after its fallback begins. So two origins reaching the same settlement class for a node can still observe that node's side effects diverge afterward. Origin invariance governs the chain's own control-flow decision, not everything the settled handler goes on to do.

Emitter-facing dispatch is acceptance-only: a call that hands a chain to the runtime accepts it or refuses it, and reports nothing about what the chain then did. A refusal is raised synchronously at the call itself, never as a value or a rejected promise the emitter could await. So no emitter-facing dispatch point yields anything an emitter could await for chain execution. Origin invariance and an awaitable emitter surface are mutually exclusive, because an awaiting emitter is itself a second, origin-specific path by which a chain's progress could be observed and acted on. Settlement observation stays internal to the executor that dispatched the chain, which is what lets the chain select its own `next` or `fallback` continuation without an emitter racing or duplicating that decision. Observing committed runtime state is a different thing and is not restricted here. A host notified that a domain's mount set has changed learns that a transition completed, not that a chain settled or which branch it selected. So such a notification is not a second path by which a chain's progress could be observed or acted on.

Per-node bounds are the whole of execution policy. Every node the executor attempts is bounded solely by that node's own declared timeout, or by the authoritative target domain's default where none is declared. The executor never rewrites that bound in light of preceding nodes, hops already crossed, transit time already spent, or how long the chain has already run. That is exactly what makes a node's bound the same wherever the chain carrying it was triggered, and it holds without any aggregate figure standing behind it. What makes the executor's settlement reachable at every node is acceptance-time refusal of a cyclic chain: a chain that would create a cycle is refused before it is accepted. An accepted chain is therefore always a finite, acyclic graph, and every path an execution follows through it is a finite sequence of individually bounded attempts. That is the whole of what finiteness rests on here, and it deliberately establishes no ceiling on a chain's total running time; nothing in this constraint entitles a reader to infer one. Where a node's target sits in another runtime, the node is handed across a hop together with its continuations by the delivering runtime, which is then done with it. It holds nothing for that node afterwards, applies no bound of its own to it, and receives nothing back. Delivery across that hop is synchronous and binary: it either refuses at the call or is accepted, never both. A refused delivery is the current action's own failure, observed and answered by the delivering runtime. An accepted node is executed, bounded, and settled entirely by the receiving registry where its target lives, which then dispatches the continuation its outcome selects from itself. Every action therefore executes at the registry authoritative for its target, under exactly the bound a locally dispatched action gets. Origin invariance thus holds for execution across hops with no transport qualification. A node whose handler is slow but inside its own bound succeeds whether it was reached locally or across any number of hops. And a node that exceeds its bound fails in both cases.

Retraction of a route and deactivation of a bridge act the same way on both sides of a hop. Each stops that route resolving for a new delivery, and neither touches a sub-chain the far side already accepted, which keeps executing there under its own rules.

Two things stay outside what this constraint settles. It does not extend origin invariance to reachability between disconnected routing trees, or between trees whose registration propagation has not yet converged; those are outside the connected-tree and converged-propagation qualifications, not a gap in them. And it does not specify a correlation protocol for a terminal completion signal. An emitter that genuinely needs to know a chain finished is expected to declare that as a node inside the chain itself, rather than by awaiting the dispatch call. That protocol's shape is separate work this constraint leaves unsettled.

**ADRs**: [The MFE Runtime's Public Access Surface](../../../architecture/ADR/0003-mfe-runtime-public-surface.md), [Host–MFE Action Dispatch and Chaining](../../../architecture/ADR/0007-action-dispatch-and-chaining.md), [Child MFE Access to the Host](../../../architecture/ADR/0008-child-mfe-host-access.md)
...
| LifecycleStage | An automatic trigger of an origin-invariant actions chain that accompanies a runtime transition — a notification that the transition is happening, not a phase the transition is composed of and not something the transition must complete before proceeding. | `LifecycleStage`, `LifecycleHook` types; dispatched alongside mount, unmount, initialization, and destruction transitions by `LifecycleManager` and `DomainLifecycleTrigger`. |

**A registration's routing identity.** Beyond the fields this runtime itself reads, an extension registration carries one value that is read outside it: the presentation route it declares (`ExtensionPresentation.route`). The navigation substrate takes that value, with one leading `/` stripped, as the occupant's own public identity token in the address bar, so it must be a single lower-case token drawn from `a`-`z`, `0`-`9` and `-`, beginning with a letter; a registration whose stripped value falls outside that alphabet, or that declares no route at all, is never projected into the address bar. This runtime neither normalizes nor checks the value, and nothing in this package depends on it. The rule, the alphabet, and the consequence of failing it are decided by `cpt-frontx-routing-adr-occupant-identity-stability` in the routing member tree ([packages/routing/architecture/ADR/0004-occupant-identity-stability.md](../../routing/architecture/ADR/0004-occupant-identity-stability.md)), which is where a consumer choosing a route value reads the full rule.
...
|----------------|---------|
| `MfeRegistry`, `MfeRegistryFactory`, `createMfeRegistryFactory`, `MountSetObserver` | The abstract registry facade, its abstract factory contract, and the creation function — the sole way a consumer obtains a registry instance bound to an injected `TypeSystemPlugin`. The concrete `DefaultMfeRegistry` / `DefaultMfeRegistryFactory` stay internal (ADR-0003, `scripts/mfes-import-boundary-check.mjs`). `MountSetObserver` is construction-time surface alongside that provider: a host may supply one when it builds a registry and the runtime notifies it whenever it commits a change to a domain's mount set, carrying the domain and what entered and left it. It reports committed runtime state only — never a chain's branch selection, settlement class, or completion — and supplying one adds no capability method to `MfeRegistry` or to either bridge, which is why it is outside what MFES-6 counts (ADR-0007). |
...
| `ActionHandler`, `ActionsChainsMediator` | The actions-chains mediator contract, exposed as an acceptance-only dispatch surface that accepts or synchronously refuses a chain and reports nothing about its subsequent execution; the default keyed-dispatch implementation stays internal (ADR-0003). |
...
sequenceDiagram
    ...
    Shell->>Shell: accept and take ownership of the chain
    ...
    Grand->>Grand: accept and take ownership of the chain
...
**Description**: A registry that composes as a mounted extension inside another registry — a host of hosts — propagates admitted targets upward through the bridge connecting it to its own host. This propagation is transitive, so every ancestor up to the shell acquires the reachability it needs, without any registry holding a reference to a non-adjacent registry. A chain the application dispatches at the shell reaches the grandchild by that same pairwise, adjacent-hop path, forwarded downward. At each hop the delivering runtime hands the node together with its continuations across, and is then done with it. A refused delivery is the current action's failure, answered from itself by the delivering runtime at that hop. An accepted node is executed by the receiving registry where its target lives, which dispatches the continuation its outcome selects from itself. A chain the grandchild dispatches with no local handler reaches the shell the same way, by a hand-over escalating upward through the same bridges in reverse. Where a continuation's target lies elsewhere again, that is a new hand-over from whichever registry just executed the action. No outcome or settlement crosses back over a completed hop in either direction. A hop at which no route resolves, or at which a resolved route's hand-over is refused, is the delivering runtime's own failure. That runtime dispatches the node's declared fallback from itself, and none of that is surfaced back to the application that dispatched the chain. This composes to any nesting depth using only the existing `ChildMfeBridge` — four capability methods plus two readonly identity properties — and two-member `ParentMfeBridge` at each boundary ([Host–MFE Action Dispatch and Chaining](../../../architecture/ADR/0007-action-dispatch-and-chaining.md), [Child MFE Access to the Host](../../../architecture/ADR/0008-child-mfe-host-access.md)).
