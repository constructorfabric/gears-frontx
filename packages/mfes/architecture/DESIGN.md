...
| ExtensionDomain | A governed placement composed with one named mount strategy (Concurrent, Optional, or Exclusive) and a declared set of lifecycle actions, validated against a cardinality matrix at registration. Optionally carries its own `route` name, registered once here instead of a host-side id-to-name table. | `ExtensionDomain` type plus `ExtensionDomainImplementation` / `ExtensionDomainImplementationFactory`. |
...
**A registration's routing identity.** Beyond the fields this runtime itself reads, an extension registration carries one value that is read outside it: its declared route — the base `Extension.route` when the registration sets one, else `presentation.route` for a screen. The navigation substrate takes that value, with one leading `/` stripped, as the occupant's own public identity token in the address bar, so it must be a single lower-case token drawn from `a`-`z`, `0`-`9` and `-`, beginning with a letter; a registration whose stripped value falls outside that alphabet, or that declares no route at all, is never projected into the address bar. This runtime validates only that a registration setting both `Extension.route` and `presentation.route` agrees on the two once each is stripped, and that a route token is unique among one domain's own registrations; it does not itself normalize the value for projection, and the navigation substrate depends on neither field's name, only on the resolved token. The alphabet and the consequence of failing it are decided by `cpt-frontx-routing-adr-occupant-identity-stability` in the routing member tree ([packages/routing/architecture/ADR/0004-occupant-identity-stability.md](../../routing/architecture/ADR/0004-occupant-identity-stability.md)); which field this runtime reads and how it reconciles the two is this runtime's own registration contract, specified in `cpt-frontx-feature-extension-domain-governance` ([features/extension-domain-governance/FEATURE.md](features/extension-domain-governance/FEATURE.md), "Route Identity").

A domain's own `route` name is validated the same way but never stripped — a domain name is not a path — and, unlike an extension's route, carries no uniqueness check of its own at this layer.
...
| `MfeError`, `DomainValidationError`, `DomainRouteValidationError`, `ExtensionRouteConflictError`, `DuplicateRouteTokenError`, `MfeLoadError`, `ExtensionTypeError`, `ChainExecutionError`, `MfeTypeConformanceError`, `UnsupportedDomainActionError`, `UnsupportedLifecycleStageError`, `EntryTypeNotHandledError` | The error hierarchy surfaced by rejection and load-failure paths, including the three route rejections at registration (invalid domain `route`, base route disagreeing with `presentation.route`, route token already held in the same domain). |
...
| `isValidRouteName`, `routeNamesEqual`, `getDeclaredRoute`, `getExtensionRouteToken` | The registration's routing identity: `name`-token validity and equality, the declared route (base `route`, else `presentation.route`), and the route token an extension is addressed by — `undefined` when it is not routable. The one definition registration enforces, kept in agreement with `@gears-frontx/routing`'s own name helpers by a test outside both packages. |
