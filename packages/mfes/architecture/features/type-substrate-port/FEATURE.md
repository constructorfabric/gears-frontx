...
11. [x] - `p1` - For the framework's well-known `destroyed` lifecycle stage, delegate resolution of its type identifier to the injected port's dedicated method, rather than the runtime holding any concrete type-format literal for this concept - `inst-resolve-lifecycle-stage-destroyed`
