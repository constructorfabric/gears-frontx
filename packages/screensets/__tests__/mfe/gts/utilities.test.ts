...
describe('Gears FrontX constants values', () => {
  ...
  describe('action schema type IDs', () => {
    ...
    it('should confirm action schema type IDs end with ~', () => {
      expect(FRONTX_ACTION_LOAD_EXT.endsWith('~')).toBe(true);
      expect(FRONTX_ACTION_MOUNT_EXT.endsWith('~')).toBe(true);
      expect(FRONTX_ACTION_UNMOUNT_EXT.endsWith('~')).toBe(true);
    });
  });
});
