...
- Developer states an intent naming only units inside ground an applied template already holds; selection offers each unit to the inventory, finds the ground's own template already applied and no further template claiming the units, so the plan carries no application at all and the units are realized through that template's activated extension skills.
...
- The host invokes the routing entry point for a request to add a unit to a project that already holds applied templates; the entry point names the same scaffolding entry point, because only that capability reads the inventory the unit may have a template of its own in.
...
2. [ ] - `p1` - The routing entry point determines which of the kit's declared capabilities the request belongs to, from the request itself and from whether the working directory already holds applied templates. It reads no installed-template inventory: classification is a decision about the request, not a selection over what is installed, and a routing step that cannot see the inventory must not decide anything the inventory answers. - `inst-rr-classify`
3. [ ] - `p1` - **IF** the request is to create a new project, **or** to add a unit to a project that already holds applied templates - `inst-rr-if-create`
   1. [ ] - `p1` - **RETURN** the scaffolding entry point (`cpt-frontx-flow-ai-project-scaffolding-scaffold-from-intent`) as the capability that serves the request. Both requests go there for the same reason: whether a named unit has a template of its own is a fact of the installed inventory, and the scaffolding flow is what reads it - it already handles a target that is already a project by omitting the seed. Routing an add-a-unit request straight to an applied template's own skills would answer the inventory's question without asking it. - `inst-rr-return-scaffolding`
...
10. [ ] - `p1` - **FOR EACH** remaining part of the intent, select at most one candidate whose declared description matches it, skipping any candidate already in the plan - a template contributes to a project once, so a part of the intent that repeats a unit the plan already covers adds no second application. A part that names a unit is matched here like any other part and is not set aside as per-unit work before it has been offered to selection, because whether a unit has a template of its own is a fact of the installed inventory and not of the unit's size. Record every part this step delivers, and the identity it selected to deliver it, as the set `inst-st-record-unit-work` excludes from; the precedence this establishes is stated in `cpt-frontx-dod-ai-project-scaffolding-single-application-per-identity`. - `inst-st-select-further`
...
11. [ ] - `p1` - Drop from the plan every selected identity already applied in the target directory, recording it as already applied, because re-applying an identity claims ground that identity already occupies and the CLI refuses the whole operation. Dropping the identity from the plan does **not** remove it, or the parts of the intent it was selected to deliver, from the exclusion set `inst-st-select-further` recorded: the identity is already applied, so the unit it claims is already delivered, and a unit delivered by an application is never per-unit work whether that application is in this plan or predates it. - `inst-st-drop-applied`
12. [ ] - `p1` - **FOR EACH** part of the intent that names a unit within a selected or already-applied template's own ground **and that the exclusion set carries no selected identity for**, record it as one unit of per-unit work **provisionally** attributed to that template - one record per unit the intent names, so an intent naming several units of one kind yields several records against the single application that owns their ground. The exclusion set is the one `inst-st-select-further` recorded and `inst-st-drop-applied` left intact, so it is read here rather than the plan: an identity dropped as already applied is absent from the plan and still delivered the unit, and reading the plan would record that unit a second time. The precedence this enforces, and why delivery by a template of its own outranks per-unit work, is stated in `cpt-frontx-dod-ai-project-scaffolding-single-application-per-identity`. Per-unit work is what is left after selection: the units for which no separate template identity exists at all. The attribution is provisional because this algorithm's only evidence is what the templates' descriptions say about the ground they establish; it holds no bundle contents. The authoritative answer comes at realization, where the covering capability is looked up by declared role in the applied template's own bundle (`cpt-frontx-flow-ai-project-scaffolding-scaffold-from-intent`), and a unit no bundle covers takes the explicit no-covering-skill branch there rather than being forced onto the template this step guessed. - `inst-st-record-unit-work`
...
14. [ ] - `p1` - **RETURN** the application plan: the seed selection if any, the ordered further templates, the identities dropped as already applied together with the parts of the intent each was selected to deliver, the per-unit work provisionally attributed to each template, and the residual intent. - `inst-st-return-plan`
...
The at-most-once rule is per identity and never per unit, so it **MUST NOT** be read as a rule that units are always realized inside an already-selected template's ground. This is the precedence rule the selection algorithm and the shipped entry-point document both enforce, and it is stated here: where the intent names a unit that a distinct installed template's own description claims, the system **MUST** offer that unit to selection before any per-unit attribution, **MUST** plan it as an application of that template, and **MUST NOT** also record it as per-unit work - including where that template is dropped from the plan because it is already applied, since a unit an application delivers is delivered whether the application is planned now or predates the plan. Delivery by a template of its own outranks per-unit work because it is what gives the unit its own declared ownership boundary, its own version line, and its own upgrade cadence, none of which a unit created inside another template's ground has; recording the unit both ways would plan it twice, once as ground to be applied and once as work to be improvised into ground. The per-unit path covers what selection could not: units for which the inventory offers no template of their own (`target`).
...
- [ ] A request to add a unit to a project that already holds applied templates is routed to the scaffolding entry point rather than to an applied template's own skills, so the unit is offered to the installed inventory before any skill is driven. (`target`)
...
- [ ] A unit the intent names that a distinct installed template's own description claims is planned as an application of that template and is not also recorded as per-unit work, including where that identity is dropped from the plan as already applied; the plan carries one application per distinct template identity, and a template identity claiming several units yields the one application that covers them, each of those units excluded from per-unit work. (`target`)
