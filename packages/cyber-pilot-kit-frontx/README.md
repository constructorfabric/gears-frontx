...
| `skills/project-scaffolding/SKILL.md` | `frontx_project_scaffolding` | skill | Builds from a stated intent, whether the target is a new project or one that already holds applied templates: matches the intent against the descriptions installed templates declare, applies the chosen set over the `frontx` command surface, then realizes each named unit no installed template claims through the applied templates' own activated skills |
