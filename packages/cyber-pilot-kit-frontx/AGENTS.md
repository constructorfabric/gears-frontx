...
- `seed` applies a template into a **new** repository - a target that does not exist, is empty, or holds only the closed set `.git`, `.DS_Store`, `Thumbs.db`; it refuses a directory holding anything else, including any regular file. `add` applies into an **existing** one. Both take the identity the template's own manifest declares, which is what `list` reports
