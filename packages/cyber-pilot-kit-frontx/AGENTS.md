...
- The command that runs the headless browser is a pinned one, `npx --yes agent-browser@<version>` or an installed binary, and every later call repeats that exact prefix. An unversioned `npx --yes agent-browser` downloads whatever release is newest on every run, so it is never used. The bare binary is not on `PATH` unless it was installed, so a call that drops the prefix fails as a command that does not exist, and six consecutive runs each rediscovered that
