...
| Add a unit to a project that already holds applied templates - one more screen, one more isolated UI unit | `frontx_project_scaffolding` - it offers the unit to the installed inventory first, because a unit may have a template of its own, and drives the skills the owning template activates under `.frontx/ai/<template-identity>/` only for a unit no installed template claims. Do not send this request to those skills directly: that answers the inventory's question without asking it. The base kit adds no unit itself |
