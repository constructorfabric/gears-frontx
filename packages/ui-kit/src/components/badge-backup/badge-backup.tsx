// Load-bearing: BadgeBackup calls useRender directly, so this can't be dropped.
...
/*
 * The pre-port Badge, kept under the `-backup` name while the canonical
 * `Badge` is a faithful port of the shadcn base badge. This one is the
 * F-mockups' Badge/Status design; it is NOT upstream-conformant and exists
 * so the semantic-intent variant set and the dot/icon slots survive the
 * re-port rather than being rediscovered from git history.
 *
 * BadgeBackup has no Base UI primitive component, but it still gets
 * `render`-prop polymorphism — from `useRender`/`mergeProps`, the same
 * utilities Base UI's primitives are built on. A status badge is usually
 * static, but rendered as a link via `render` it stays focusable and gets
 ...
 * rule is real, but it lives in badge-backup.md and in the VALUE names —
 * the prop itself keeps the kit-wide `variant` name rather than an axis name
 * of its own, so every component in the kit is driven the same way.
 *
 * `shape`, not `size`: the two values are pill vs. plain (bare text, no
 * fill), and BadgeBackup has no size axis at all (the pill is a fixed 24px).
...
export interface BadgeBackupProps
...
export function BadgeBackup({ className, variant, shape, dot, icon, render, children, ...props }: BadgeBackupProps) {
