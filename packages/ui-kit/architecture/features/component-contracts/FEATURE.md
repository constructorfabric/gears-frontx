...
2. [x] - `p1` - **FOR EACH** exported declaration, keep the ones that are React components, unwrapping a wrapper by its resolved symbol rather than by the name it was imported under. A component takes one of three forms: a body carrying markup; a body returning the primitive library's render hook - the kit's own polymorphism mechanism, where the hook returns the element and the body carries no markup at all - resolved by the hook's symbol rather than by the name at the call site, for the same reason a wrapper is; or an alias of a component-typed callable, which has no body to read at all and is the form every directory's root takes. An alias is a component when the type of the name it aliases has a call signature that can return a React element, React's own element type resolved by symbol the way a heritage reference is. Renderability alone is not that test, because a string is renderable and a formatter is not a component, and the test is asked only of a body-less alias, so a helper with a body is never swept in by its return type - `inst-ex-candidates`
