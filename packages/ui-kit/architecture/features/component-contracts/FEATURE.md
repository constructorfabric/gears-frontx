...
2. [x] - `p1` - **FOR EACH** exported declaration, keep the ones that are React components, unwrapping a wrapper by its resolved symbol rather than by the name it was imported under. A declaration renders when its body carries markup, and equally when it returns the primitive library's render hook - the kit's own polymorphism mechanism, where the hook returns the element and the body carries no markup at all - resolved by the hook's symbol rather than by the name at the call site, for the same reason a wrapper is - `inst-ex-candidates`
