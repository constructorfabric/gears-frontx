...
3. [x] - `p1` - Report a property of a DECLARED prop the schema does not state in full and that has no slot record, and a slot record on a property the schema DOES state in full or on a prop the component does not declare. Scoped to declared props: a prop of the primitive underneath can also assert nothing, and its type is documented in its own description and its own untyped statement rather than in the kit's slot record - `inst-fr-slots`
...
6. [x] - `p1` - Assert the pairing between the properties a validator will not fully check and the contract's untyped statements about a prop, both ways - `inst-cf-untyped`
...
- [x] Every property that asserts nothing is named by exactly one untyped statement about a prop, and every such statement names a property that asserts nothing - checked both ways on every described component's run, keyed on what each statement is about.
...
- [x] Every untyped statement says what it is about, from the closed list, and the one about a single property names that property; a statement about anything else carries no property name.
