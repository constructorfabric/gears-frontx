...
**Input**: The conflict-cleared staged assembly (per-template contributions and declared ownership boundaries, including `region-union` shared-file entries with their owned region keys, per `cpt-frontx-feature-template-manifest`), the target repository path, a target-file reader, and existing provenance records whose structured occupied ownership boundaries record already-applied region-union ownership by template identity, file path, and region key (`cpt-frontx-feature-composed-provenance`) — empty when the target is a fresh seed.

**Output**: For each repository file path, the materialized file body — a single owner's content for an `exclusive` path or exclusive subtree, or a marker-delimited append-safe host document for a `region-union` path — written to the target repository; a materialization-invariant error if a declared-level collision (same region key, or a contested `exclusive` path) reaches this stage; a materialization refusal, writing no file, when a `region-union` host carries a begin OR end marker with no parseable `identity:key` token, a begin marker with no matching end marker before end of file, or an end marker that never closes any located block (no matching begin marker before it, or a begin marker whose region a different, earlier end marker already closed) — that marker's block boundaries cannot be established for ANY block on the path, so the host cannot yet be classified as carried-forward, stale, or unrecorded; a materialization refusal, writing no file, when the host document has duplicate contributor-owned marker blocks or overlapping/nested marker spans before stale contributor blocks could be dropped; a materialization refusal, writing no file, when a file already on disk at a `region-union` path carries a marker block that is neither owned by a current contributor nor recorded in structured provenance for the same template identity, path, and region key; a materialization refusal, writing no file, when carried-forward blocks read from a `region-union` host are internally inconsistent — two of them resolving the same region key, regardless of identity; a materialization refusal, writing no file, when a contributor's declared owned region cannot be located exactly once in that template's installed content; a host-document conflict, writing no file, when a non-host contributor supplies conflicting unmarked host text; or a materialization conflict refusing the assembly when any two extracted owned regions on the path — across templates or within a single template's multiple keys — have overlapping actual on-disk marker spans (the content-level check the pre-flight cannot perform).
...
3. [x] - `p1` - **FOR EACH** target file path with any `region-union` contribution — one contributor or many - `inst-cs-foreach-multi`
   ...
   2. [x] - `p1` - Choose the marker-delimited host document: the file already on disk at the target path when one exists, otherwise the first `region-union` contributor's installed content for the path; locate every begin/end sentinel-marker pair on that host, recording each located block's owning identity, region key, span, and verbatim text — the region-addressing schema `cpt-frontx-feature-template-manifest` owns, applied here in reverse to discover identity-and-key pairs from the host rather than to locate one already-known pair. - `inst-cs-read-existing-blocks`
   3. [x] - `p1` - **IF** the chosen host document carries a begin or end marker with no parseable `identity:key` token, a begin marker with no matching end marker before end of file, or an end marker that closes no located block — either because no begin marker for its `identity:key` precedes it, or because an earlier begin marker for that same `identity:key` already claimed the nearest preceding available end marker, leaving this one unclaimed - `inst-cs-if-malformed-marker`
   ...
   4. [x] - `p1` - **IF** the chosen host document — the file already on disk, or the seed contributor's installed content when no file exists — has duplicate contributor-owned marker blocks, or any two located host marker blocks have overlapping or nested spans - `inst-cs-if-host-document-conflict`
      1. [x] - `p1` - **RETURN** a host-document materialization refusal naming the path and offending marker ownership, and write no file. This runs before stale contributor-owned blocks are dropped, carried blocks are trusted, or seed-host blocks are extracted, so malformed host structure cannot be hidden by replacement semantics. - `inst-cs-return-host-document-conflict`
   5. [x] - `p1` - **IF** a located block's owning identity is neither a contributing template in the staged assembly nor recorded in the target repository's existing structured provenance for the same template identity, path, and region key - `inst-cs-if-unrecorded-block-owner`
      1. [x] - `p1` - **RETURN** a materialization refusal naming the path, the unrecorded owning identity, and the region key, and write no file. The unrecorded owner has no declaration in the comparison set the pre-flight conflict check (`cpt-frontx-algo-cli-scaffolding-conflict-check`) evaluated, so its block is ground no arbitrated claim accounts for; composing over it would either drop the occupying template's contribution or silently absorb an un-arbitrated claim, and `cpt-frontx-adr-assembly-conflict-prevention` forbids both outcomes. The block being present on disk is evidence the occupied-boundary picture was incomplete — it is NOT a declaration of ownership (`cpt-frontx-adr-template-ownership-boundary-declaration` rejects inferred ownership from emitted output). The refusal states how to bring the repository's provenance into agreement — recording the owning template's applied provenance with structured path/key ownership — and retry. - `inst-cs-return-unrecorded-owner`
   6. [x] - `p1` - Carry forward, verbatim from disk — never re-derived from installed content — every located block whose owning template identity, path, and region key are recorded in the target repository's existing structured provenance and whose owning identity is not a contributing template in the staged assembly: this operation materializes only the newly applied templates' contribution (`cpt-frontx-flow-cli-scaffolding-add-template`), and that block's key was already arbitrated by the pre-flight conflict check via the occupied-boundary comparison. - `inst-cs-carry-forward-recorded-blocks`
   7. [x] - `p1` - **IF** two or more carried-forward blocks resolve the same region key — regardless of whether they share an owning identity, since a region key is unique per shared-file path, not per identity, mirroring `inst-cc-if-region-key-clash` and `inst-cs-if-carried-key-collision` — checked before any lookup keyed by region key is built from them, so a duplicate cannot be silently dropped - `inst-cs-if-carried-block-conflict`
      1. [x] - `p1` - **RETURN** a materialization refusal naming the path, the identities and region key(s) involved, and whether a duplicate key was found; write no file. Every carried-forward block was located in the ONE on-disk buffer read at `inst-cs-read-existing-blocks`, which the pre-flight conflict check (`cpt-frontx-algo-cli-scaffolding-conflict-check`) never inspects at all — so, unlike `inst-cs-if-carried-key-collision` below, this is not a condition the pre-flight check could have refused. It is evidence the target file was edited by hand or otherwise corrupted since this tool last wrote it, the same untrusted-on-disk-content concern `inst-cs-if-unrecorded-block-owner` already guards. - `inst-cs-return-carried-block-conflict`
   8. [x] - `p1` - **FOR EACH** contributing template, verify its non-empty unmarked host-document text is byte-identical to the other contributors' non-empty host skeletons, while preserving the already-on-disk host text as the materialization base when the file exists; then locate and extract each declared owned region exactly once from its installed content by matching the begin/end sentinel markers keyed by that template's identity and declared region key (region-addressing schema owned by `cpt-frontx-feature-template-manifest`). - `inst-cs-extract-regions`
      1. [x] - `p1` - **IF** a contributor supplies conflicting unmarked host text, or any declared region is missing, duplicated, malformed, unterminated, or otherwise unlocatable in that contributor's installed content - `inst-cs-if-contributor-region-invalid`
         1. [x] - `p1` - **RETURN** a host-document or region materialization refusal naming the path, template identity, region key where applicable, and marker defect; write no file. A declared `ownedRegions` key must resolve to exactly one marker block, never to an omitted contribution. - `inst-cs-return-contributor-region-invalid`
   9. [x] - `p1` - **IF** two contributors resolved the same declared region key on the path - `inst-cs-if-key-collision`
   ...
   11. [x] - `p1` - **IF** any two extracted regions on the path have overlapping actual on-disk marker spans — whether owned by different templates or by the same template declaring multiple region keys - `inst-cs-if-span-overlap`
   ...
   12. [x] - `p1` - Compose the repository file's materialized body by preserving the chosen host document's unmarked text and existing marker-block order, replacing host blocks whose region key has a freshly extracted contributor block, keeping recorded carried-forward blocks verbatim, dropping stale contributor-owned host blocks that are no longer declared, and appending regions absent from the host in deterministic owning-identity/region-key order. - `inst-cs-compose-union`
   13. [x] - `p1` - Record the composed content as the path's materialized body — not written to the target repository yet. - `inst-cs-write-composed`
...
5. [x] - `p1` - **FROM** RESOLVED **TO** ABORTED **WHEN** the add flow's target path exists and is not a directory after the template set has resolved but before the pre-flight conflict check runs; no files are written. - `inst-as-resolved-aborted-target-not-directory`
6. [x] - `p1` - **FROM** RESOLVED **TO** ABORTED **WHEN** the add flow's target holds content at ground the staged assembly would write and no existing provenance accounts for that ground before the pre-flight conflict check runs; no files are written. - `inst-as-resolved-aborted-ground-occupied`
7. [x] - `p1` - **FROM** RESOLVED **TO** CONFLICT_CHECKED **WHEN** the pre-flight conflict check finds no intersecting boundary claim across the staged assembly and any already-occupied boundaries. - `inst-as-resolved-checked`
8. [x] - `p1` - **FROM** RESOLVED **TO** ABORTED **WHEN** the pre-flight conflict check reports an intersecting boundary claim; no files are written. - `inst-as-resolved-aborted-conflict`
9. [x] - `p1` - **FROM** CONFLICT_CHECKED **TO** ABORTED **WHEN** the add flow's last-moment target recheck finds the target path is no longer a directory; no files are written. - `inst-as-checked-aborted-target-not-directory`
10. [x] - `p1` - **FROM** CONFLICT_CHECKED **TO** ABORTED **WHEN** the add flow's last-moment target recheck finds content at ground the staged assembly would write and no existing provenance accounts for that ground; no files are written. - `inst-as-checked-aborted-ground-occupied`
11. [x] - `p1` - **FROM** CONFLICT_CHECKED **TO** ASSEMBLED **WHEN** the cleared assembly is materialized into the target repository and one provenance record is written per applied template. - `inst-as-checked-assembled`
...
The system **MUST** materialize a `region-union` shared file as a marker-delimited append-safe host document: choose the file already on disk as the host when it exists, otherwise seed from the first contributing `region-union` file; preserve unmarked host text; replace or insert each contributing template's declared marker block, extracted exactly once from its installed content by the identity-and-region-key sentinel markers whose schema `cpt-frontx-feature-template-manifest` owns; carry forward recorded non-contributor marker blocks; and preserve region markers for later boundary-scoped upgrade. An `exclusive` path is written whole by its single owner. Declared-level collisions (same region key, or a contested `exclusive` path) reaching materialization are invariant violations because the pre-flight conflict check already refuses them, whereas an overlap between any two extracted owned regions' actual on-disk marker spans — whether contributed by different templates or by a single template declaring multiple keys — is a content-level property the pre-flight cannot observe, so materialization (run per shared-file path regardless of contributor count) is the authority that detects it and refuses the assembly (`target`).
...
The system **MUST**, when materializing a `region-union` shared file, carry forward verbatim — from the file already on disk, never re-derived from installed content — every marker-delimited block whose owning template identity, file path, and region key are recorded in the target repository's existing structured provenance and whose owning identity is not a contributing template in the staged assembly, and **MUST** refuse the whole assembly, writing no file, when the file already on disk carries a block that is neither owned by a current contributor nor recorded by that same identity/path/key tuple — because an on-disk block is evidence the occupied-boundary picture was incomplete, never itself a declaration of ownership. The system **MUST** also refuse the whole assembly, writing no file, when the carried-forward blocks themselves are internally inconsistent — two of them resolving the same region key, regardless of whether they share an owning identity — trusting the file already on disk no more than `inst-cs-if-unrecorded-block-owner` does, since that file can be edited by hand between applies (`target`). The system **MUST** refuse the whole assembly, writing no file, EVEN EARLIER — before either block-owner check trusts the chosen host's shape at all — when the chosen host document, whether read from disk or seeded from the first contributor, carries duplicate contributor-owned marker blocks, overlapping or nested marker spans, a begin or end marker with no parseable `identity:key` token, a begin marker with no matching end marker before end of file, or an end marker that closes no located block (no preceding begin marker for its `identity:key`, or its nearest preceding available end marker was already claimed by an earlier begin marker sharing that same `identity:key`): such a marker's block boundaries cannot be established for ANY block on the path, contributor-owned or not, naming the path, the marker's line number where applicable, and the marker defect (`inst-cs-if-malformed-marker`, `inst-cs-if-host-document-conflict`, `target`).
...
- [ ] A shared file co-owned by two or more applied templates under `region-union` is materialized as a marker-delimited append-safe host document: unmarked host text is preserved, each contributing template's declared region is extracted exactly once from installed content by the identity-and-region-key sentinel markers, existing host blocks are replaced or left carried forward as appropriate, absent regions are appended deterministically, and markers are preserved; an `exclusive` path is written whole by its single owner. (`target`)
...
- [ ] Adding a template into a repository whose existing structured provenance records a `region-union` block by the same template identity, file path, and region key that this add's staged assembly does not contribute materializes that shared file with the recorded block carried forward verbatim from disk, alongside the newly applied template's own region, rather than truncating the file to only the new contribution. (`target`)
- [ ] Materialization refuses the assembly, writing no file, when a `region-union` path already on disk carries a marker block that is neither owned by a current contributor nor recorded in the target repository's existing structured provenance for the same template identity, path, and region key. (`target`)
- [ ] Materialization refuses the assembly, writing no file, when the carried-forward blocks read from a `region-union` path already on disk are internally inconsistent — two of them resolving the same region key, regardless of whether they share an owning identity — regardless of whether the pre-flight conflict check found any declared-boundary collision. (`target`)
