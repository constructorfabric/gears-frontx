...
- Every entry in every set carries `description`, the same manifest-declared field that carries selection semantics (`cpt-frontx-adr-template-manifest-contract`). Under the current manifest contract `description` is required and non-empty (`cpt-frontx-feature-template-manifest`), so a conforming entry always declares one; **the key is absent, never empty and never a placeholder, only for a damaged installed entry** - one whose stored manifest satisfied the required-description contract when it was installed (a legacy manifest, carrying no such field or any other retired field, is refused outright at install and never enters local inventory, `cpt-frontx-algo-template-manifest-refuse-legacy`), but whose on-disk copy has since been corrupted or manually altered - a consumer selects templates by this value, and a substituted placeholder would be indistinguishable from a declaration the template never made.
