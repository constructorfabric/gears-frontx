...
8. [x] - `p1` - Read the template's manifest from the acquired content — a manifest discriminated as legacy by the presence of any retired field is read only through the isolated migration path (`cpt-frontx-algo-template-manifest-migrate-legacy`, `cpt-frontx-feature-template-manifest`), never directly by the primary four-field type — and take the identity it declares as the template's identity - `inst-resolve-name`
