...
9. [ ] - `p1` - **FOR EACH** declared `excludedSubtrees` entry - `inst-for-each-excluded-subtree`
   1. [ ] - `p1` - **IF** the entry is not a well-formed target-relative path, or contains a `..` segment - `inst-if-excluded-subtree-malformed`
      1. [ ] - `p1` - Add violation: an `excludedSubtrees` entry must be a well-formed target-relative path with no `..` segment - relative to the template's own target, not to the repository the template is eventually applied into, since the manifest is authored before any target is known - `inst-add-excluded-subtree-malformed-violation`
