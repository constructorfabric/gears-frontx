...
The system **MUST** ensure a manifest carrying any retired field (`schemaVersion`; `ownershipBoundaries` or its `exclusiveSubtrees`/`sharedFiles` children; `referencedTemplates`) is refused with `INVALID_MANIFEST` by every command that reads a manifest, including `install` — never read through a translation into `excludedSubtrees`, and never accepted regardless of whether its retired `exclusiveSubtrees`/`sharedFiles` were already effectively whole-target or its `description` is usable. There is no partial credit and no migrated output (`target`).
