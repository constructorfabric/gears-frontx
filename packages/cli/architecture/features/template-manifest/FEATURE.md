...
9. [ ] - `p1` - **FOR EACH** declared `excludedSubtrees` entry - `inst-for-each-excluded-subtree`
   1. [ ] - `p1` - **IF** the entry is not a well-formed target-relative path, or contains a `..` segment - `inst-if-excluded-subtree-malformed`
      1. [ ] - `p1` - Add violation: an `excludedSubtrees` entry must be a well-formed target-relative directory path with no `..` segment - relative to the template's own target, not to the repository the template is eventually applied into, since the manifest is authored before any target is known; a glob pattern or a path naming a file rather than a directory is likewise malformed, since the entry reserves ground for a nested template's target, not a hook into a file's own content - `inst-add-excluded-subtree-malformed-violation`
...
3. [ ] - `p2` - **ELSE** (at least one retired field is present — a legacy manifest) - `inst-mml-else-legacy`
   ...
   2. [ ] - `p2` - Set `ownership.excludedSubtrees` to the empty list: the retired `exclusiveSubtrees` and `sharedFiles` categories have no faithful translation into the current "directory reserved for a nested template" semantics — a shared-file region names files, not a subtree — so neither retired category is carried forward, and the migrated template's effective ownership becomes the current model's whole-target default acting on its entire legacy target - `inst-mml-map-ownership`
   3. [ ] - `p2` - **IF** the manifest declares a usable, non-empty `description` - `inst-mml-if-description`
      1. [ ] - `p2` - Carry it across unchanged into the current shape's `description` - `inst-mml-map-description`
   4. [ ] - `p2` - **ELSE** (no usable `description`) - `inst-mml-else-no-description`
   ...
   5. [ ] - `p2` - **RETURN** the current four-field shape — `name`, `version`, `description`, and the always-empty `ownership.excludedSubtrees` — with no retired field carried onto it - `inst-mml-return-migrated`
...
- [ ] A manifest whose `ownership.excludedSubtrees` declares an entry that is not a well-formed target-relative path, contains a `..` segment, or is not a strict descendant of the template's own target (including an entry that coincides with the target itself), causes a FAIL naming each offending entry.
