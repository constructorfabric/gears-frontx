...
**Output**: The current four-field shape (`name`, `version`, `description`, `ownership.excludedSubtrees`) ready for the primary manifest type, unchanged for an already-current manifest or migrated for a legacy one; or a validation refusal — naming the missing `description`, or naming the untranslatable retired ownership categories — for a legacy manifest this path cannot honestly migrate. `install` may still proceed against a migrated shape that carries a usable `description`, while `register` fail-closes on either refusal (`cpt-frontx-adr-thin-template-manifest`).
