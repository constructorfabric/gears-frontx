...
- [3. Processes / Business Logic (CDSL)](#3-processes--business-logic-cdsl)
  ...
  - [Read a Legacy Manifest Through the Isolated Migration Path](#read-a-legacy-manifest-through-the-isolated-migration-path)
...
- [5. Definitions of Done](#5-definitions-of-done)
  ...
  - [Legacy Migration Never Widens Ownership Silently](#legacy-migration-never-widens-ownership-silently)
...
**AI-extension bundle write path - resolved.** `.frontx` is unconditionally excluded from every template's effective ownership under whole-target ownership (`cpt-frontx-adr-whole-target-ownership`), so a template's manifest cannot claim any path under `.frontx` - including an identity-scoped AI-extension bundle root such as `.frontx/ai/<manifest-name>/` - as an `ownership.excludedSubtrees` entry or through any other category this contract declares. That write path is not left to this contract: a template's payload may carry a conventional `.frontx/ai/<manifest-name>/` folder at the root of the template directory (per `cpt-frontx-feature-template-manifest`'s own payload definition, §1.2), and a dedicated CLI-owned step this feature does not own copies that folder out to the project's `.frontx/ai/<manifest-name>/` the first time the name gains an applied target (`cpt-frontx-algo-cli-scaffolding-ai-bundle`, `cpt-frontx-feature-cli-scaffolding`) - never through this manifest's `ownership.excludedSubtrees` or any category it declares.

**Reserved CLI-owned `.frontx/` namespace.** `.frontx` as a whole holds CLI-owned metadata, never template content: the single project state document, `.frontx/project.json` (schema owned by `cpt-frontx-feature-composed-provenance`), is the CLI's own record of a project's registered templates, their origins and versions, and their applied targets, and is written only by the CLI's own commands. Because whole-target ownership already subtracts `.frontx` unconditionally from every template's effective ownership (`cpt-frontx-adr-whole-target-ownership`, CLI-5), this manifest contract has no declared category through which a template could claim ground under `.frontx` in the first place; this paragraph documents that reservation as a fact of the domain model rather than as a further rule this feature's validation algorithm enforces.
...
- **ADR**: `cpt-frontx-adr-thin-template-manifest`, `cpt-frontx-adr-whole-target-ownership`
...
### Read a Legacy Manifest Through the Isolated Migration Path

- [ ] `p2` - **ID**: `cpt-frontx-algo-template-manifest-migrate-legacy`

Closes the read-side counterpart to pre-publish validation: a manifest published under the retired ADR 0018 five-category shape must remain readable at install time even though the primary manifest type declares none of that shape's retired fields (`cpt-frontx-adr-thin-template-manifest`). This is the one isolated migration path every manifest-reading command (install, register, apply, assembly) funnels a legacy manifest through; its output is always the current four-field shape or a validation refusal — the retired fields never reach the type any command reads, whether directly (a current manifest) or through this path (a legacy one).

**Input**: A parsed manifest JSON structure.

**Output**: The current four-field shape (`name`, `version`, `description`, `ownership.excludedSubtrees`) ready for the primary manifest type, unchanged for an already-current manifest or migrated for a legacy one; or one of two validation refusals for a legacy manifest this path cannot honestly migrate. The two refusals are not equivalent in which commands they block: a missing `description` still yields a usable four-field shape with that key simply absent, so `install` may proceed against it (reporting no `description`) while only `register` fail-closes; untranslatable retired ownership categories yield no four-field shape at all — there is nothing to install — so this refusal blocks `install` as well as `register` (`cpt-frontx-adr-thin-template-manifest`).

**Steps**:
1. [ ] - `p2` - Determine whether the manifest carries at least one retired field (`schemaVersion`; `ownershipBoundaries` or its `exclusiveSubtrees`/`sharedFiles` children; `referencedTemplates`) — the discriminator `cpt-frontx-adr-thin-template-manifest` fixes for "legacy" versus "current" - `inst-mml-discriminate`
2. [ ] - `p2` - **IF** no retired field is present - `inst-mml-if-current`
   1. [ ] - `p2` - **RETURN** the manifest unchanged; a current-shape manifest never enters this migration path - `inst-mml-return-current`
3. [ ] - `p2` - **ELSE** (at least one retired field is present — a legacy manifest) - `inst-mml-else-legacy`
   1. [ ] - `p2` - Map `name` and `version` across unchanged into the current shape - `inst-mml-map-identity`
   2. [ ] - `p2` - **IF** the manifest's retired `ownershipBoundaries.exclusiveSubtrees` is absent, empty, or exactly `["."]`, and its retired `ownershipBoundaries.sharedFiles` is absent or empty — the legacy declaration already claimed the whole legacy target with no shared-file regions, so the current model's whole-target default is not a widening but the same claim restated - `inst-mml-if-ownership-lossless`
      1. [ ] - `p2` - Set `ownership.excludedSubtrees` to the empty list: this translation is exact, not an approximation - `inst-mml-map-ownership-lossless`
   3. [ ] - `p2` - **ELSE** (the retired `exclusiveSubtrees` names a genuine proper subset of the legacy target — the common case for a template that declared what it owns narrowly, such as a 30-entry whitelist that never claimed everything under its target — or the retired `sharedFiles` is non-empty) - `inst-mml-else-ownership-lossy`
      1. [ ] - `p2` - **RETURN** a validation refusal (`INVALID_MANIFEST`) naming the untranslatable retired categories: a whitelist narrower than the whole target has no faithful blacklist equivalent — the current model's only ownership vocabulary is "whole target minus these exclusions," and there is no exclusion list that reproduces a *narrower*-than-whole claim — and a shared-file region names files, not a subtree. Never silently widen a legacy template's effective ownership to its entire target on the strength of a translation that cannot represent what it actually claimed; the refusal directs the template's author to convert it manually, choosing a deliberate `ownership.excludedSubtrees` for the current shape, the same conversion path `cpt-frontx-feature-template-territory-conversion` already performs for this repository's own templates - `inst-mml-return-ownership-lossy`
   4. [ ] - `p2` - **IF** the manifest declares a usable, non-empty `description` - `inst-mml-if-description`
      1. [ ] - `p2` - Carry it across unchanged into the current shape's `description` - `inst-mml-map-description`
   5. [ ] - `p2` - **ELSE** (no usable `description`) - `inst-mml-else-no-description`
      1. [ ] - `p2` - **RETURN** a validation refusal naming the missing `description` — `install` may still read the migrated shape absent a caller-imposed requirement, while `register` fail-closes on this refusal, never registering a name with no usable description (`cpt-frontx-adr-thin-template-manifest`) - `inst-mml-return-no-description`
   6. [ ] - `p2` - **RETURN** the current four-field shape — `name`, `version`, `description`, and the losslessly-translated (never widened) `ownership.excludedSubtrees` — with no retired field carried onto it - `inst-mml-return-migrated`
...
**Constraints**: none directly owned; `cpt-frontx-adr-thin-template-manifest` governs contract evolution
...
The system **MUST** ensure the same manifest shape (`cpt-frontx-contract-template-manifest`) - exactly `name`, `version`, `ownership.excludedSubtrees`, and a required non-empty `description` - that the pre-publish validate command checks is the shape consumed at install, register, apply, and assembly time; there is exactly one descriptor per template, no per-command divergence, and no command reads a different or partial descriptor. Unlike the retired five-category shape, `description` is now required: a manifest omitting it, or declaring it as anything other than a non-empty string, is rejected (`target`). A manifest published under the retired five-category shape **MUST** reach this same current shape only through the isolated migration path (`cpt-frontx-algo-template-manifest-migrate-legacy`), never by admitting a retired field onto the primary type: `install` **MUST** accept a migrated legacy manifest that carries a usable description, and `register` **MUST** fail-close on one that does not (`cpt-frontx-adr-thin-template-manifest`).

**Implements**:
- `cpt-frontx-algo-template-manifest-validate-contract`
- `cpt-frontx-algo-template-manifest-migrate-legacy`
...
### Legacy Migration Never Widens Ownership Silently

- [ ] `p2` - **ID**: `cpt-frontx-dod-template-manifest-migrate-ownership-fidelity`

The system **MUST** ensure the legacy migration path (`cpt-frontx-algo-template-manifest-migrate-legacy`) never produces a migrated manifest whose effective ownership claims more of its target than the legacy manifest actually claimed. When the legacy `exclusiveSubtrees` is absent, empty, or exactly `["."]` and `sharedFiles` is empty, the translation to an empty `ownership.excludedSubtrees` **MUST** proceed, because it is exact. Otherwise — a genuine proper-subset `exclusiveSubtrees`, or a non-empty `sharedFiles` — migration **MUST** refuse with `INVALID_MANIFEST` rather than translate, since no `excludedSubtrees` value can represent a narrower-than-whole claim (`target`).

**Implements**:
- `cpt-frontx-algo-template-manifest-migrate-legacy`

**Constraints**: none directly owned; `cpt-frontx-adr-thin-template-manifest` governs the migration path's existence and its fail-closed posture

**Touches**:
- CLI: install command, register command (`target`)
- Entities: `TemplateManifest`, `OwnershipBoundary`
- Component: `cpt-frontx-component-cli`
...
- [x] The manifest shape is versioned so that previously published manifests remain readable when the contract evolves - a manifest published under the retired five-category shape is read only through an isolated migration path that produces the current four-field shape, or refuses when its retired ownership categories cannot be translated without widening them, per `cpt-frontx-adr-thin-template-manifest`.
- [ ] Per the CI fixture `cpt-frontx-adr-thin-template-manifest`'s Confirmation fixes: a legacy manifest (discriminated by any retired field) with no `description` migrates and `install` succeeds against the migrated shape, while `register` refuses it until a `description` is added — verifiable via `cpt-frontx-algo-template-manifest-migrate-legacy`.
- [ ] A legacy manifest whose retired `exclusiveSubtrees` is absent, empty, or exactly `["."]`, and whose `sharedFiles` is empty, migrates to an empty `ownership.excludedSubtrees` and is registrable — the translation is exact, not a widening.
- [ ] A legacy manifest whose retired `exclusiveSubtrees` names a genuine proper subset of its target (for example, a 30-entry whitelist that never claims its entire target) — the shape `template-shell`'s own pre-conversion manifest had — refuses migration with `INVALID_MANIFEST` rather than silently widening to whole-target ownership; the same refusal applies to a legacy manifest whose `sharedFiles` is non-empty.
