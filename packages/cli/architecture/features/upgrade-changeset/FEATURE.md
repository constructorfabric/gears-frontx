...
**Steps**:
1. [x] - `p1` - Compute the target's effective ownership boundary once, from the candidate manifest's declared `excludedSubtrees` together with `projectOwnedRoots`, `.frontx`, the reserved environment entries, and the template's own local origin folder — the same six-term subtraction `cpt-frontx-algo-cli-scaffolding-uniform-apply` performs (`inst-ua-compute-ownership`) — and subtract, on top of it, every OTHER registered template's local origin folder, exactly as the apply path does in its own separate step (`inst-ua-subtract-other-origins`). That term is named here rather than left implied in the citation: without it an upgrade could classify a path inside a sibling template's own source folder as `ADD`/`REPLACE`/`REMOVE` and write through it, which is the hazard that step was added to `apply` to prevent — never the baseline manifest's `excludedSubtrees`, and never recomputed per path - `inst-cls-compute-candidate-boundary`
...
6. [x] - `p1` - **FOR EACH** enumerated path - `inst-cls-foreach-path`
   ...
   2. [x] - `p1` - **IF** this path is carried by the baseline payload or the candidate payload, and the disk holds a directory or a symlink at this exact path instead of a regular file or an absence, **OR** any directory component of the path strictly between `target` and the leaf is itself reported as a symlink — the same extension `cpt-frontx-algo-cli-scaffolding-existing-content`'s own `inst-ec-if-symlink-component` already makes on the apply side, applied here in this algorithm's own terms rather than imported, because that helper probes one full-tree walk while this algorithm probes one injected `readDiskEntry(absolutePath)` call per path: a symlinked ANCESTOR is invisible to a leaf-only `lstat`, since the OS transparently resolves every intermediate path segment on both a read and a write, so without this clause a symlinked ancestor directory would let this step compare — and the commit algorithm land — content at wherever the link actually points, never at the path the plan names - `inst-cls-if-not-regular`
      1. [x] - `p1` - Record the path as doubly-changed for this target, fail-closed, for either cause; no comparison is attempted - `inst-cls-record-not-regular`
...
- [x] A payload path where the disk holds a directory or a symlink instead of a regular file refuses fail-closed with `CONTENT_CONFLICT`, with no comparison attempted for that path; the identical refusal applies when any directory component between the target and that path — not the path itself — is a symlink, whether it resolves inside the target, inside the project but outside the target, outside the project, or nowhere at all (a dangling link), because such a component makes the OS resolve the path elsewhere than where the plan names it, on both the classification read and, absent this refusal, the eventual write.
...
- [x] A developer's own file, symlink, or ordinary directory sitting inside a target's effective ownership at a path neither payload carries is never enumerated, never compared, never reported in the plan, and never refuses the upgrade — the plan carries only paths a payload declares, so it stays reviewable as the handful of operations the upgrade actually performs; a payload path where the disk holds a directory or a symlink instead of a regular file, or where any directory component below the target that a payload path passes through is itself a symlink, still refuses fail-closed — including when an ordinary internal symlink elsewhere in the project (a `node_modules` link, a symlinked build output) sits outside every enumerated payload path's own ancestor chain, which is never probed and so never refuses anything.
