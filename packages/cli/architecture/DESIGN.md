...
| `cpt-frontx-fr-cli-template-boundary-declaration` | A template's effective ownership is computed algorithmically rather than read from a separately declared boundary record: it is the whole target minus the six subtractions CLI-5 (§2.2) enumerates, and the assembler and the conflict checker both derive it from the manifest and the project state store. CLI-5 is the single enumeration; this row names the rule, not a second copy of its terms. |
...
| `cpt-frontx-fr-cli-upgrade-restore` | An applied upgrade's commit is a standing requirement to remain reversible to the prior applied state; the concrete restore mechanism is intentionally left open by `cpt-frontx-adr-project-upgrade-mechanism` for a dedicated future decision (§4). |
| `cpt-frontx-fr-cli-template-delete` | The assembler's `delete` path computes the deletion plan as the target's effective ownership (CLI-5's six subtractions, never a subset of them) minus any nested target belonging to another template, through the conflict checker's geometry; it requires explicit confirmation defaulting to no, supports a non-destructive `--dry-run` preview, and removes the target from the project state store on success. Deletion is bounded by effective ownership rather than by its own separate subtraction list, so a term added to CLI-5 is a term `delete` stops removing. |
...
**ADRs**: [How a Template Source Is Referenced and What a Stored Reference Holds](../../../architecture/ADR/0017-source-spec-syntax.md)
...
| OwnershipBoundary | A template's effective claim over its own target: the whole target minus its manifest-declared `excludedSubtrees`, minus the project's `projectOwnedRoots`, minus its own local origin folder, minus `.frontx`, minus the environment-owned entries (`.git`, `.DS_Store`, `Thumbs.db`) — computed algorithmically rather than separately declared, and compared across the whole batch plus already-applied state to detect a conflicting assembly. | Computed at check time from the manifest and the project state document — role owned by this DESIGN, decision by [How a Template Declares the Boundaries of What It Owns](../../../architecture/ADR/0031-template-ownership-boundary-declaration.md), concrete schema owned by [features/template-manifest/FEATURE.md](features/template-manifest/FEATURE.md) |
...
- Owns `delete`: computes a target's deletion plan as that target's effective ownership (CLI-5's six subtractions, applied whole) minus any nested target belonging to another template, through the conflict checker's canonicalized geometry, and executes it only under explicit confirmation (defaulting to no) or the JSON envelope's `CONFIRMATION_REQUIRED` protocol, with a non-destructive `--dry-run` preview of exactly what would be removed and preserved.
