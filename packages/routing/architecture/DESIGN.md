...
- Signals the host-injected mounting executor to unmount any currently mounted owner that is not the owner the current resolution named — not only an owner whose declared prefix no longer contains the pathname, which under legal prefix nesting can under-unmount (see `cpt-frontx-algo-routing-url-screen-binding-unmount-on-prefix-exit` in the url-screen-binding FEATURE).
...
- **Location**: `src/index.ts` (to be authored against this contract; no source exists yet).
...
sequenceDiagram
    ...
    Binding->>OwnerPort: resolve pathname against declared prefixes (longest match)
    OwnerPort-->>Binding: route owner (or none)
