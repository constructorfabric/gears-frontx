...
| Level | One extension domain's own position in the zone tree, and the point at which resolution and observation both run: a level's own base, its own declared route-owner pairs, and its own axis (or axes) are all local to it, never shared with another level. | Resolved per domain — `@gears-frontx/routing` |
| Base | The concrete URL value a level's own resolution runs relative to, supplied to it by the enclosing level the moment it begins to exist — a render-time value, never a declaration carried in a registration. Only the outermost level's base is an absolute prefix of the browser's own location. | Consumer-supplied per level |
| Axis | A domain's own projection strategy into the URL: axial, continuing the pathname (at most one per zone), or parallel, occupying a dedicated query-string key whose entry multiplicity follows that domain's own occupancy strategy. | Consumer-assigned per domain, opt-in |
...
- **Contracts**: the substrate's own `NavigationHistory` contract (`location`, `subscribe`, `push`, `replace`, `go`); the `basepath` contract; the consumer-supplied owner-prefix pairs source, the per-level/per-axis observer's own construction shape, and the route ownership signal's transition shape (field-level shapes owned by the route-ownership-signal FEATURE); the engine-provider port a provider package must satisfy — its concrete shape is stated abstractly here and owned in full by the engine-provider FEATURE that defines it (now published in the provider's own package tree).
...
| URL back-projection helper | The helper a consumer calls to reflect a mount not driven by navigation back into the URL — replacing the pathname beneath an axial domain's own base, or replacing only a parallel axis's own query-string key entries, leaving the pathname, every other key, and (by default) the hash and the search untouched or reset per the axis's own rule. Field-level shape owned by the route-ownership-signal FEATURE. |
...
sequenceDiagram
    ...
    L0-->>Glue0: report transition (appeared: Owner A)
    ...
    Ax-->>Glue0: report transition for the parallel axis (appeared: Owner C, or none)
    Note over Glue0,Glue1: Each level's and each axis's own wave resolves independently; no participant here asserts the whole path resolved together.
```
