...
The shell subroute — everything between the first `/` and `?` — is owned entirely by the shell (the host application); this package never interprets it and copies it verbatim on every write. A shell subroute containing `?`, `#`, or `&` cannot be written: `serializeGrammar` throws `invalid-shell-subroute` rather than silently producing a URL that would reparse into a different, corrupted structure the moment it is read back — a shell subroute produced by this package's own parse can never contain one of these three characters by construction (parse cuts it off at the first `?`), so this rule binds only a caller that constructs a serialize input directly. The hash is likewise the application's own, copied verbatim and never interpreted. The query string is where this package reads and writes structure; occupants supply the payload values inside it.
...
A segment of the query string that does not parse as an entry at all — no `=` between a candidate domain key and extension, or a candidate domain key carrying a token outside the `name` alphabet or an even number of segments — is not this package's own business: it is kept, verbatim, as a **foreign segment**, silently, with no warning, and re-emitted unchanged on the next write, after every entry, in its own original relative order among the other foreign segments — not necessarily interleaved with entries the way the source query string had them, since this package does not track that original interleaving once parse separates the two lists. A segment whose candidate domain key does conform to the `domain-key` production but that still fails to become an entry — an invalid extension token, or a param breaking the entry rules further in (a malformed percent-escape, an empty param name) — warns only when its shape is unmistakably an entry of this grammar: the domain key already carries the ancestry form (a `.`-composed key), or the raw segment carries `;` parameters. A param breaking the entry rules further in only ever arises once `;` parameters are present, so it always meets this bar and always warns. A single-segment key with no `;` parameters is exactly as shaped as an unrelated query parameter, and this package has no registry of the ecosystem's actual domain keys to tell the two apart, so an invalid extension under a bare `key=value` segment is foreign, silently, on the same terms as a plainly foreign one, however malformed the extension looks. When a segment does warn, it is dropped from the parsed entry list and reported to the host as a warning, and it is *also* kept as a foreign segment, on the same terms as a plainly foreign one. A segment that does parse as a well-formed entry but is dropped only for colliding with an earlier one under the same domain key (the same-extension collision below) is neither warned as malformed nor kept as foreign — it already is an entry; the rule that drops it is a different, unrelated one. Every other entry in the same query string is kept, and parsing never throws. An empty raw entry — produced by two consecutive `&` characters (`a=b&&c=d`) or a trailing `&` — is silently ignored and dropped with no warning, unlike every other malformed case above, and is not collected as a foreign segment either — there is no text to keep. With zero entries and zero foreign segments across every domain, the serializer emits the shell subroute alone, with no trailing `?`; a query string that is present but empty (`/en?`) parses to that same zero-entry, zero-foreign-segment state and re-serializes identically to the bare shell subroute. A query string carrying only foreign segments (zero entries, but at least one segment kept as foreign) still needs the `?`, because there is content in it even though no domain projects an entry, and round-trips through parse then serialize byte-exactly, since foreign segments re-emit in their own original relative order with nothing else to place them among.
