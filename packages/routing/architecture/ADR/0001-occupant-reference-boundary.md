...
* **Occupant as a structural parameter contract, adapted by a separate glue layer** — the routing core resolves against `Occupant`: a structural type carrying a stable identity plus an opaque parameter bag, with no knowledge of `mfes`'s `Extension` shape, passed as a plain argument at each resolution call rather than injected; a glue/adapter layer, architecturally the same role the default engine provider's own Engine Provider already plays for the router engine, maps concrete `Extension` registrations into that structural shape.
...
* Bad, because that glue is not a nameless abstraction for this ecosystem: it is the host's glue layer, which is exactly why this package's own `cpt-frontx-routing-adr-occupant-identity-stability` fixes which concrete extension field the glue layer reads as identity — that obligation is decided and enforced there, not restated here — while which extension types are routable at all is the runtime's own registration contract, recorded as an assumption in this package's PRD, not a decision this ADR restates either.
...
**Scope of impact.** Governs only how the routing core (`@gears-frontx/routing`) names and carries route-owner identity through resolution and reporting. It does not decide how a concrete glue layer maps `mfes` `Extension` registrations into the Occupant contract — that mapping is the glue layer's own concern, outside this package's boundary — nor does it revisit the engine-provider port this ADR draws its contrast from (`cpt-frontx-routing-fr-engine-provider-port`). It also does not decide, and defers entirely, which concrete extension field the glue layer reads as identity — this package's own `cpt-frontx-routing-adr-occupant-identity-stability` decides that — nor which extension types carry a route at all, which the runtime's own registration contract decides and this package's PRD records only as an assumption. The lexical well-formedness rule for an occupant's own identity token — the `name`-alphabet shape it must satisfy to serve as a URL entry's extension token, and the synchronous validation this package runs at observer creation and at the URL back-projection helper — is recorded in `cpt-frontx-routing-adr-domain-occupancy-addressing-granularity` (ADR 0003), the record that owns the grammar's own `name` alphabet, not restated here.
...
**Occupant is assembled from two inputs, not handed over pre-built.** "Passed as a plain argument at each resolution call" describes the mechanism — argument-passing rather than injection — not the timing or the source of every field Occupant carries. In the route-ownership-signal FEATURE's own resolution primitive, a domain's own registered-extensions source supplies only the identity half of Occupant — the extension token each Route Owner declares, with no payload attached to it — at observer creation and, when the source exposes one, on its own change notification; the payload half is never carried by that source at all. It always arrives separately, from the URL entry the resolution primitive matches against, at resolution time (route-ownership-signal FEATURE §1.5, Registered-extensions source and Transition; `cpt-frontx-algo-routing-route-ownership-signal-entry-resolution`). An Occupant the resolution primitive names — identity plus payload together — is therefore assembled from two different inputs arriving through two different channels, not handed to the routing core as one pre-built pair at one call site. The two input paths this record's own Consequences names as where lexical well-formedness is checked — observer creation and the back-projection helper — are joined by two more validating paths this package's own artifacts record elsewhere: the domain-key-composition function, which validates the extension token it is given as one of the three parts a nested domain's own key is composed from (`cpt-frontx-routing-adr-domain-occupancy-addressing-granularity`), and the registered-extensions source's own change notification, which validates every token in the newly-current set before re-running resolution against it (route-ownership-signal FEATURE, §1.5, Registered-extensions source).
...
* `cpt-frontx-routing-fr-route-ownership-signal` — the signal reports Occupant identity, not concrete `Extension` identity, at every domain.
...
* `cpt-frontx-routing-adr-occupant-identity-stability` — this package's own decision that fixes which concrete extension field the glue layer reads as Occupant identity; this ADR names that record as where that obligation is decided and enforced, not restated. Which extension types carry a route at all is, in turn, the runtime's own registration contract's decision — recorded as an assumption in this package's PRD, not by an ADR this package holds.
* `cpt-frontx-routing-adr-domain-occupancy-addressing-granularity` — the source of the grammar's `name` alphabet, and the record that states the Occupant Identity Lexical Rule normatively, in its own Decision Outcome, for the allowed identity character set.
