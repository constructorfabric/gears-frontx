...
- A root domain's own name is the host's own single choice for each domain it projects: the `route` field on the domain's own registration, owned by the `mfes` runtime (`ExtensionDomain.route`), validated against the `name` alphabet at registration time and rejected if it fails. This library treats the name as a plain argument the host supplies, never as a value it derives.
