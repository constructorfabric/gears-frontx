...
3. [ ] - `p1` - Call the shared history's `replace` — never `push` — with the given prefix as the pathname, carrying the search and hash read in step 2 forward onto it unchanged, the same location-preserving behavior `cpt-frontx-routing-fr-location-preserving-helpers` provides elsewhere in this package - `inst-call-replace`
