...
Route Ownership Signal publishes small, independent facts derived from the URL at one domain level, and orchestrates no mounting. First, it exposes the navigation substrate's own longest-matching-prefix primitive as this package's public owner-resolution surface: given a domain level's own local remainder and that domain's own set of declared identifier-to-prefix pairs, it names the owner and its declared prefix that matched, or reports that none matches; a pure function, not stateful, and not a re-implementation of the substrate's matching rule. Second, it exposes an observable signal a consumer creates once per domain level and once per axis that level projects — passing that domain's own owner-prefix pairs source as a plain argument, never an injected port, together with the base within that level's own carrier that its enclosing level handed it (the pathname, for a level whose own zone continues the main axis; a named query-string key's own current entry value, for a level whose own zone continues that key's value instead): the observer subscribes to the navigation substrate's shared history and, on creation and on every subsequent navigation, reports a transition from the previously resolved owner to the newly resolved one at that level, distinguishing an owner appearing, an owner disappearing, an owner changing, or the same owner's own remainder beneath its unchanged, single declared prefix changing — or no transition at all when none of those changed, which is also what a navigation that changes only a foreign carrier — another axis's own key, or the hash — produces, since resolution matches on that level's own local remainder within its own carrier alone. Creating the observer also returns a release function the consumer calls to stop it, unsubscribing everything the observer itself subscribed to. Third, it exposes a URL back-projection helper a consumer calls when a mount happened for a reason other than navigation: one mode replaces the remainder beneath a level's own base within its own carrier — the pathname, or a parallel-axis key's own value — the other replaces an entire parallel-axis key's own entry outright, leaving everything else untouched. None of the three touches mounting, unmounting, a registry of owners, or a state machine tracking which owner currently occupies a placement, at any domain level — those facts and that occupancy belong entirely to whichever mount mechanism the consumer already runs.
...
**Binding obligation**: Two-way agreement between the URL and the mounted route owner is not a guarantee this package makes on its own, at any domain level — it is a joint guarantee of this package plus the consumer's own host glue holding that level. PRD §3.1 records it as a deployment obligation resting on the consumer rather than a library capability, exactly as it records the deployment obligations `cpt-frontx-feature-routing-engine-provider` places on a consumer rather than on that feature itself. This obligation is recursive: whichever consumer holds a domain level's own observer carries it for that level, with no lesser or different obligation for a level several domains deep than for the outermost one. A consumer that wants the URL and the mounted owner to durably agree, at a domain level it holds:
1. Creates the observer described in §3 (Observable Owner-Change Signal) for that level and for each axis it projects, supplying that domain's own owner-prefix pairs source, together with the base within that level's own carrier the enclosing level handed it.
...
- A level's own base is handed to it by its enclosing level at creation, and that same enclosing level owns re-handing it: this feature's own observer never updates a base in place (§3, Observable Owner-Change Signal — Base-mismatch note). When the enclosing level's own engine re-matches the parametric value a nested level's base was built from to a different value, the enclosing level's own consumer completes releasing the observer holding the stale base before creating the fresh one carrying the new value — never creating the successor while the stale observer's own subscription is still live, since having both alive at once in the same round would let the enclosing level receive contradicting reports from each. This is release and recreate, in that order, never a protocol for updating the base of an observer still running, because a base is immutable by construction for the lifetime of the observer it was handed to.
- Unmounting the occupant that owns a zone releases every observer this feature created for a domain within that zone, through the same consumer glue that created them — nested first, outermost last, or in whatever order that glue already tears down the zone's own contents. An observer left running past its own zone's unmount continues resolving and reporting against a base and a set of declared pairs whose owning zone no longer exists, reporting to a consumer that itself no longer has anything meaningful to do with the report.
...
- A readable snapshot of the current set of declared identifier-to-prefix pairs for that one domain, in the same shape the navigation substrate's owner-resolution primitive reads (`cpt-frontx-algo-routing-navigation-substrate-prefix-resolution`); no identifier appears against two different prefixes (PRD §11).
...
- New owner's own declared prefix (or none): carried as a convenience for the consumer's own mount mechanism — it is an ingredient of the base that owner's own zone hands to whatever domain nests inside it, combined with the base this level's own observer was itself handed and whichever parametric segment the owner's own engine-provider package consumes beneath its prefix (PRD glossary, `base`) — even though the prefix itself is always derivable from the new owner's own identifier alone, since a route owner declares exactly one prefix for as long as its registration exists (PRD §11). Absent when the new owner is "no owner".
- Kind: `appeared` (no owner → an owner), `disappeared` (an owner → no owner), `changed` (one owner → a different owner), or `remainder-changed` (the same owner persists, and its one declared prefix is therefore necessarily unchanged too, but the portion of the local remainder beneath that prefix changed). No transition is delivered at all — not a further Kind value, an absence of delivery — when the owner is "no owner" both before and after, or when the same owner persists with no change to the local remainder beneath its prefix; the latter is what a navigation that changes only a foreign carrier produces — the current URL's search or hash, for an observer whose own carrier is the pathname, or any query-string key other than its own, for an observer whose own carrier is a parallel-axis key — since resolution matches on that observer's own local remainder within its own carrier alone (§3, Observable Owner-Change Signal).
...
**URL back-projection — axial mode input**: a specific declared prefix to reflect as the remainder beneath a level's own base, within that level's own carrier — the current pathname, or a parallel-axis key's own current entry value — together with that base itself, exactly as the enclosing level handed it; the caller supplies the exact prefix, which is always the mounted owner's own single declared prefix (PRD §11). By default this mode does not carry the current search or hash forward, when the carrier is the pathname: replacing that carrier's own remainder resets both, exactly as the core navigation substrate's own `push`/`replace` carry nothing forward that the caller does not itself compose into the given location. A caller that wants the current search, or the current hash, carried forward reads either one and passes it explicitly alongside the prefix; carrying forward a foreign axis's own search key selectively is not offered, because doing so would require this helper to know that key's own identity and semantics, which it does not — state that must survive this reflection regardless of which axis changed is a shared property, not something either back-projection mode carries (§1.4, Binding obligation). The helper reports nothing back to the caller beyond letting the underlying `replace` call either succeed or throw whatever the shared history itself throws.

**URL back-projection — parallel-axis mode input**: a named query-string key and the specific local path to reflect into that key's own entry — this release's own projection of a parallel axis carries at most one entry per key, zero or one, never more than one at a time (DESIGN §1.1, Axes within a zone; `cpt-frontx-adr-extension-domain-occupancy`) — the caller supplies the exact path, mounted for the axis's own occupant. This mode mutates only that key's own entry: it leaves the pathname, every other query-string key, and the hash exactly as they were, never resetting or carrying anything beyond the one key it was asked to change. The helper reports nothing back to the caller beyond letting the underlying `replace` call either succeed or throw whatever the shared history itself throws.
...
**Steps**:
1. [ ] - `p1` - The consumer creates the observer for a domain level, passing that domain's own owner-prefix pairs source together with the base within its own carrier its enclosing level handed it - `inst-create-observer`
   1. [ ] - `p1` - The observer reads the current location from the shared history and resolves that level's own local remainder's owner through the owner resolution primitive (`cpt-frontx-algo-routing-route-ownership-signal-owner-resolution`) - `inst-resolve-owner-at-creation`
   2. [ ] - `p1` - **IF** an owner resolves - `inst-if-owner-resolves-at-creation`
      1. [ ] - `p1` - The observer reports an initial transition of kind `appeared` to the consumer's registered callback (`cpt-frontx-algo-routing-route-ownership-signal-observe-change`) - `inst-report-initial-appeared`
   3. [ ] - `p1` - **ELSE** no owner resolves at creation - `inst-else-no-owner-at-creation`
      1. [ ] - `p1` - The observer reports no initial transition — there is nothing yet to report a change from (§1.4, Binding obligation point 5) - `inst-initial-no-report`
...
3. [ ] - `p1` - **IF** the transition reports an owner appearing, or changing to a not-yet-mounted owner - `inst-if-appeared`
   ...
   2. [ ] - `p1` - Once mounted, the microfrontend's own engine provider reads the current location from the shared history at start and matches the remainder of the path under its base, as specified by whichever engine-provider package implements the port - `inst-mfe-reads-location-at-start`
...
**Input**: A domain level's own owner-prefix pairs source, passed as a plain argument at creation (never an injected port); this level's own carrier and the base within it that its enclosing level handed it at that same moment — the current pathname and a position inside it, for a level whose own zone continues the main axis, or a named query-string key and a position inside its own current entry value, for a level whose own zone continues that key's value instead, with an empty base denoting the value's own root for a level projected at the top of its own parallel axis; a callback the consumer registers to receive a transition; the navigation substrate's shared history and its fan-out subscription; the observer's own internally tracked previous owner and previous local remainder, updated after every resolution this algorithm performs.

**Output**: An observer subscribed to the navigation substrate's fan-out, plus a release function returned to the consumer at creation (`cpt-frontx-algo-routing-route-ownership-signal-release`); on creation, and on every subsequent navigation that is ownership-relevant to this level's own carrier, one call to the consumer's callback carrying an ownership-change transition (§1.5, Ownership-Change Transition); no call at all when neither the owner nor the local remainder beneath it changed.

**Steps**:
1. [ ] - `p1` - **WHEN** the consumer creates the observer, passing that domain's own owner-prefix pairs source, its own carrier (the pathname, or a named query-string key), and the base within that carrier it was handed, and registering its callback - `inst-when-create`
   1. [ ] - `p1` - Read the current local remainder for this level's own carrier: the portion of the carrier's current value beneath the given base — the current pathname's own portion beneath the base, for a level whose own zone continues the main axis, or the named key's own current entry value beneath the base, for a level whose own zone continues that key's value (the whole entry, when the base is empty) - `inst-read-initial-remainder`
...
2. [ ] - `p1` - **WHEN** the navigation substrate's fan-out notifies the observer of a navigation - `inst-when-navigation`
   1. [ ] - `p1` - Read the current local remainder for this level's own carrier, exactly as step 1.1 did - `inst-read-current-remainder`
   ...
   7. [ ] - `p1` - **ELSE IF** the previous and new owner are the same declared owner, and the current local remainder equals the previously recorded one - `inst-elseif-same-owner-same-remainder`
      1. [ ] - `p1` - Report no transition — nothing about this level's own local remainder changed; for a level whose own carrier is the pathname, this is, in particular, what a navigation that changes only the current URL's search or hash produces, since resolution matches on the pathname remainder alone; for a level whose own carrier is a parallel-axis key, it is what a change to any key other than its own produces - `inst-no-report-unchanged`
...
**Base-mismatch note**: A level's own base is fixed at this observer's own creation (§1.4, Binding obligation) and neither step 1 nor step 2 ever updates it in place. If the carrier's current value stops carrying that base as its own prefix at all — most commonly because a parametric segment the base was built from re-matches to a different value — step 1.1 or step 2.1 reads an empty local remainder: there is nothing beneath a base the carrier's current value no longer starts with. The owner resolution primitive then resolves that empty remainder exactly like any other unmatched remainder, returning "no owner", and steps 2.3–2.9 report whatever transition that implies (typically `disappeared`, if an owner was previously bound) exactly as they would for any other legitimate absence. The same empty-remainder outcome follows when this level's own carrier is a parallel-axis key that is absent from the current search altogether, not merely present with an empty value (Absent-key note, below) — an absent key is not a separate, undefined case; it is read no differently than any other base the carrier's current value no longer carries as a prefix. This is ordinary operation, not a fault condition: an observer whose base has gone stale honestly reports the absence its own, unchanged base now implies, and relies on its own level's enclosing consumer to release it and create its replacement carrying the base's new value (§1.4, Binding obligation) — this algorithm itself has no mechanism, and no need for one, to detect that its own base has gone stale.

**Absent-key note**: For a level whose own carrier is a parallel-axis key, the key may be entirely absent from the current search — not merely present with an empty value — which is the stationary state of any parallel-axis occupant that has already closed, and the state the worked example's own `disappeared` transition arises from (DESIGN §4, Worked Example). Step 1.1 and step 2.1 read an absent key exactly as an empty entry value: the local remainder beneath any base is then empty, and the owner resolution primitive resolves it to "no owner" regardless of what the base itself is. An absent key is therefore not a distinct, undefined case for this algorithm to define separately.

**Rationale**: Because a route owner declares exactly one prefix for as long as its registration exists (PRD §11), the previously recorded owner alone is enough to tell an owner change from a remainder change — no separate previously-recorded-prefix state is needed. Reporting a `remainder-changed` transition rather than staying silent when the owner does not change is what lets a consumer whose own mount mechanism cares about in-territory navigation — for example, to keep an outer layout in sync — react without this feature inventing knowledge of what that reaction should be; the consumer's own mechanism decides what, if anything, to do with it. Tracking the previously recorded local remainder, not only the previously recorded owner, is what makes that reporting precise: without it, a navigation that leaves this level's own remainder exactly as it was — most notably one that changes only a foreign carrier — would be indistinguishable from one that actually moved within the owner's own territory. Filtering out a "no owner" to "no owner" transition, and a same-owner/same-remainder transition, are the two cases this algorithm treats as not ownership-relevant: nothing about occupancy could have changed when no declared prefix matched before or after, or when the local remainder beneath a persisting owner's one prefix is unchanged. Reporting an initial transition at creation, rather than only from the first subsequent navigation, is what covers a cold load or a reload — the very case a consumer most needs this signal for — from the first moment the observer exists. Generalizing the observer's own input to a carrier and a base within it — rather than hardwiring the carrier to the pathname or reserving a base only for the outermost case — is what lets the same algorithm serve a level on the main axis, a level projected at the top of a parallel axis, and a level nested inside one, without this feature defining three.
...
**Input**: A specific declared prefix the consumer wants reflected as the remainder beneath a level's own base, within that level's own carrier — the pathname, for a level whose own zone continues the main axis, or a parallel-axis key's own current entry value, for a level whose own zone continues that key's own value instead — mounted for a reason other than a navigation-driven transition this feature's observer reported; the base itself, exactly as the enclosing level handed it to this level's own observer at creation; optionally, when this level's own carrier is the pathname, the current location's search and/or hash, when the caller chooses to carry either forward.

**Output**: A `replace` call against the shared history whose target carrier keeps its own value unchanged up to and including the given base, carries the given declared prefix as the remainder beneath that base, and discards whatever the carrier held deeper than the base beforehand; when the carrier is the pathname, the search and the hash are reset to empty unless the caller supplied either to carry forward, in which case exactly what the caller supplied is carried, nothing more; when the carrier is a parallel-axis key's own entry value, the pathname, every other query-string key, and the hash are left exactly as they were.

**Steps**:
1. [ ] - `p1` - The consumer calls this helper with a specific declared prefix — the mounted owner's own single declared prefix (PRD §11) — and this level's own base, when its own mount mechanism performed a mount that was not initiated by a navigation-driven transition from the observer (§3, Observable Owner-Change Signal) - `inst-consumer-calls-helper`
2. [ ] - `p1` - Read the current value of this level's own carrier: the current pathname, for a level whose own zone continues the main axis, or the parallel-axis key's own current entry value, for a level whose own zone continues that key's own value - `inst-read-current-carrier`
3. [ ] - `p1` - **IF** this level's own carrier is a parallel-axis key's own entry value, not the pathname - `inst-if-carrier-is-parallel-key`
   1. [ ] - `p1` - Read the current location's pathname, every query-string key other than this one, and the hash — the parts the composed `replace` must leave untouched, mirroring the equivalent read in the parallel-axis back-projection algorithm (`cpt-frontx-algo-routing-route-ownership-signal-parallel-axis-back-projection`) - `inst-read-rest-of-location`
4. [ ] - `p1` - Keep the portion of the carrier's value read in step 2 up to and including the given base, discarding whatever the carrier held deeper than the base - `inst-keep-up-to-base`
5. [ ] - `p1` - Substitute the given declared prefix as the remainder beneath the kept base, in place of whatever remainder the carrier previously held there - `inst-substitute-remainder`
6. [ ] - `p1` - **IF** the caller supplied a search and/or a hash to carry forward (meaningful only when this level's own carrier is the pathname) - `inst-if-caller-supplied-carryover`
   1. [ ] - `p1` - Read exactly the value the caller supplied for each - `inst-read-caller-supplied-carryover`
7. [ ] - `p1` - **ELSE** the caller supplied neither, or this level's own carrier is not the pathname - `inst-else-no-carryover`
   1. [ ] - `p1` - Treat the search and the hash as reset, when this level's own carrier is the pathname — the default, since nothing is carried forward that the caller does not itself supply; when this level's own carrier is a parallel-axis key, the pathname and the hash read in step 3.1 stay exactly as they were instead, since this mode never resets a carrier it does not own - `inst-default-reset`
8. [ ] - `p1` - Call the shared history's `replace` — never `push` — writing the value composed in steps 4–5 back into this level's own carrier (the pathname, or the named parallel-axis key's own entry, whichever carrier this level has); when the carrier is the pathname, together with whatever search and hash resulted from step 6 or step 7; when the carrier is a parallel-axis key, together with the pathname, every other query-string key, and the hash read in step 3.1, unchanged - `inst-call-replace`
9. [ ] - `p1` - **RETURN** - `inst-return-reflected`

**Timing note**: This helper's guarantee — that a `replace` issued eagerly still lands on the browser's own current, post-move entry — holds for a user's own back/forward step, and for a third-party history move that has already completed by the time this helper is called: in both cases the browser's own location has already settled to its post-move value before any of this library's own code runs, even though this library's own notification of that move still lags asynchronously behind it (`cpt-frontx-algo-routing-navigation-substrate-fanout-dispatch`). It does not hold for a `go` call issued through this same navigation-substrate instance: unlike a user's own gesture or an already-completed third-party move, this instance's own `go` call does not complete synchronously at its own call site — the browser processes the move, and raises the event this library's fan-out observes, only afterward. Calling this helper in the window between this instance's own `go` call and the fan-out's own notification of the move that call produced is therefore not a supported use: which entry the browser has actually reached at that moment is undefined from this helper's own point of view, and this library does not resolve it — a consumer does not call this helper during that window. A consumer that invokes this helper eagerly for a user's own back/forward step, or for an already-completed third-party move, before observing the transition that move itself produces, still replaces the post-move entry; a consumer that wants to reflect a mount into the URL after issuing its own `go` call instead waits for the transition that call produces (§3, Observable Owner-Change Signal) before calling this helper, exactly as it would for any other navigation-driven mount.

**Rationale**: Keeping the carrier's own value up to and including the base, rather than replacing the whole carrier outright, is what keeps a nested level's own back-projection from erasing whatever an enclosing level's own occupant already established above it — the base is exactly the boundary between what an enclosing level owns and what this level is reflecting, and it is why this algorithm takes the base as an explicit input rather than assuming the whole carrier belongs to it. `replace` rather than `push` is what keeps a mount the user did not navigate to from creating an extra back/forward entry: unlike `push`, a `replace` does not truncate the forward portion of the history stack after a back step the user has already taken, so the reflection does not discard forward history the way an added entry would — the URL stays authoritative without the reflection itself becoming a navigation the user has to step back through. This has a real, accepted cost: `replace` overwrites the history entry the user was previously on, so that entry becomes unreachable by a subsequent back step, and any remainder deeper than the base that entry carried in this level's own carrier is discarded along with it. That cost is the price of treating the URL as authoritative, not an oversight of this algorithm — the alternative, `push`, would avoid the cost but let a mount the user never navigated to create a back/forward entry they now have to step through, which is the worse trade this algorithm exists to avoid. Resetting the search and the hash by default, rather than carrying them forward unconditionally, follows directly from the core navigation substrate's own signature: `push` and `replace` accept whatever location string the caller composes, and carry forward nothing the caller did not itself put there (DESIGN §1.1) — a default carry-forward was only ever a convenience layered on top, and it stops being a safe default once a parallel axis's own search key becomes a first-class occupant of the search string, since an unconditional carry-forward would silently revive whatever that key held. A caller for which total carryover is actually wanted reads the current search (and the hash, if wanted) and supplies it explicitly (§1.5) — nothing about this default forecloses that; carrying forward one foreign axis's own key selectively, rather than the whole search, is not offered by this helper at all, because doing so would require knowing that key's own identity and semantics, which this algorithm does not have and should not need — a consumer with state that must survive this reflection regardless of which axis changed holds it as a shared property instead (§1.4, Binding obligation). This default's reset scope is the whole search, not only the reflecting level's own remainder: when a level several domains deep mounts for a reason other than navigation and its own consumer calls this helper without explicitly carrying the current search forward, every parallel axis's own key currently in the search is reset along with it, including one projected by a wholly unrelated, shallower zone — a console-level modal open at the moment a deep, non-navigational mount reflects itself closes right along with it, unless the caller carrying out that reflection explicitly reads and re-supplies the modal's own key. This is the accepted price of the reset-by-default rule stated above, not a side effect this algorithm failed to anticipate: the alternative — carrying every foreign key forward by default — is exactly the unconditional carry-forward already rejected earlier in this rationale, for the same reason.
...
**Input**: A named query-string key and the specific local path the consumer wants reflected into that key's own entry, mounted for a reason other than a navigation-driven transition this feature's observer reported; the current location's pathname, every other query-string key, and the hash, all read unchanged.

**Output**: A `replace` call against the shared history that mutates only the given key's own entry to the given local path, leaving the pathname, every other query-string key, and the hash exactly as they were.
...
3. [ ] - `p1` - Compose a location carrying the pathname, every other key, and the hash read in step 2 unchanged, with the given key's own entry set to the given local path - `inst-compose-location-own-key-only`
...
**Rationale**: This mode mutates only the one key it was asked to change, never resetting or carrying forward anything else, because a parallel axis's own occupant is never the only thing the URL is currently representing — a level continuing the main axis's own pathname, and every sibling parallel axis's own key, are independent facts this helper has no business disturbing when it was asked to reflect only one of them. This mode is the case of the generalized back-projection algorithm (`cpt-frontx-algo-routing-route-ownership-signal-url-back-projection`) where the level's own base within its own carrier — this named key's own value — is empty: nothing precedes the reflected remainder, so keeping "up to and including the base" keeps nothing, and the composition reduces to replacing the key's entire entry outright. It is kept as its own named algorithm because its own input never needs a base parameter at all, only the key and the path to reflect, and because its own guarantee is narrower than that algorithm's own reset-by-default rule for the pathname: that algorithm resets what it does not carry because replacing the pathname's own remainder is inherently touching the whole location's dominant fact, while this mode never had a "whole location" to reset in the first place — only its own one key. The replace-not-push reasoning both modes share is owned in full by the other algorithm, not repeated here.
...
The system **MUST** provide a helper that replaces — never pushes — a level's own carrier (the pathname, or a parallel-axis key's own entry value, whichever carrier that level continues) so it keeps its own value unchanged up to and including a given base and carries a given, specific declared prefix as the remainder beneath that base, discarding whatever remainder the carrier held there beforehand; **MUST**, when the carrier is the pathname, reset the current search and hash by default rather than carrying either forward, and **MUST** carry forward exactly whatever search and/or hash the caller explicitly supplies instead; callable by a consumer whenever its own mount mechanism mounted an owner for a reason other than a navigation-driven transition this feature's observer reported, accepting as the deliberate cost of that guarantee that the replaced entry becomes unreachable by a subsequent back step.
...
- [ ] A consumer creates the observer, per domain level and per axis that level projects, by passing that domain's own owner-prefix pairs source as a plain argument, never through an injected port, together with the base within that level's own carrier it was handed.
...
- [ ] No transition is reported when the resolved owner is "no owner" both before and after a navigation, nor when the same owner persists with the local remainder beneath it unchanged; for a level whose own carrier is the pathname, a navigation that changes only the current URL's search or hash falls into this latter case, and for a level whose own carrier is a parallel-axis key, a change to any key other than its own does, because resolution matches on that observer's own local remainder alone.
...
- [ ] The axial-mode URL back-projection helper replaces — never pushes — a level's own carrier (the pathname, or a parallel-axis key's own entry value, whichever carrier that level continues), keeping the carrier's own value unchanged up to and including a given base and carrying a given, specific declared prefix as the remainder beneath it, discarding whatever remainder the carrier held there beforehand; when the carrier is the pathname, it resets the current search and hash by default and carries forward exactly whatever search and/or hash the caller explicitly supplies instead; the replaced entry becomes unreachable by a subsequent back step as the accepted cost of that guarantee, and a `replace`, unlike a `push`, does not truncate the forward portion of the history stack after a back step already taken. Invoking the helper before the fan-out has delivered the notification for a user's own back/forward step, or for an already-completed third-party move, still replaces the entry the browser has already moved to, not the entry current before that move; invoking it in the window between this instance's own `go` call and the fan-out's own notification of that call is not a supported use, since that call has not necessarily completed synchronously at its own call site.
- [ ] The parallel-axis-mode URL back-projection helper replaces — never pushes — only a named query-string key's own entry with a given local path, leaving the pathname, every other query-string key, and the hash exactly as they were.
...
- [ ] A level's own base is fixed at its observer's own creation and is never updated on a live observer; when its own carrier's current value stops carrying that base as a prefix at all, the observer resolves an empty local remainder to "no owner" and reports the ownership-relevant transition that implies, as ordinary operation rather than an error. The enclosing level that handed the base is the only party that ever replaces it, and only by releasing the observer holding the stale value and creating a new one carrying the updated value — never by updating the base of the observer already running.
- [ ] Unmounting the occupant that owns a zone releases every observer this feature created for a domain within that zone, through the same consumer glue that created them, so no observer keeps resolving and reporting against a base or a set of declared pairs whose owning zone no longer exists.
