...
2. On each notified transition, mounts or unmounts the named owner through its own mount mechanism — resolving any race between two mounts the same way it already resolves one for that mechanism; this feature resolves no mount race itself, and a mount driven by this signal is racing against, and resolved by, exactly the same mechanism a mount driven by any other trigger already is. A `prefix-changed` transition (§1.5, Kind) is a mounting transition for this obligation, not an in-territory one: the same owner persists, but the prefix that matched changed, so the consumer remounts that owner under the newly matched prefix as its `basepath` — the instance already mounted was built for the previous `basepath` and is not addressed by the new one, and must not be left running under a stale scope. Only `remainder-changed` is the in-territory case that requires no mounting action; `prefix-changed` requires exactly the mounting action `appeared` requires, differing only in that an instance of the same owner was already mounted under a different `basepath`.
...
- Previous matched prefix (or none): the specific declared prefix that produced the previous resolution — present whenever a previous owner was resolved, absent when the previous owner was "no owner"; carried so a consumer can tell which prefix a `prefix-changed` transition (below) moved away from.
- New matched prefix (or none): the specific declared prefix that produced the new owner's resolution — carried because a single owner may declare more than one prefix (PRD §11), so the owner's identifier alone does not say which one matched; absent when the new owner is "no owner".
- Kind: `appeared` (no owner → an owner), `disappeared` (an owner → no owner), `changed` (one owner → a different owner), `prefix-changed` (the same owner persists, but its matched prefix itself changed — the owner declared more than one prefix and the URL now matches a different one of them), or `remainder-changed` (the same owner and the same matched prefix persist, but the portion of the pathname beneath that prefix changed). `prefix-changed` and `remainder-changed` are mutually exclusive and together cover every case where the same owner persists: the first is a change of *which* declared prefix matched — the consumer's mounted instance was scoped to the old one and is not addressed by the new one — the second is a change confined *beneath* an unchanged matched prefix, which the already-mounted instance's own routing table absorbs. No transition is delivered at all — not a further Kind value, an absence of delivery — when the owner is "no owner" both before and after, or when the same owner and the same matched prefix persist with no change to the pathname beneath it; the latter is what a navigation that changes only the current URL's search or hash produces, since resolution matches on pathname alone (§3, Observable Owner-Change Signal).
...
- A URL under a different prefix declared by the same, already-mounted owner is opened; the observer reports a `prefix-changed` transition and the consumer's own mount mechanism remounts that owner under the newly matched prefix, exactly as it would a fresh mount.
...
4. [ ] - `p1` - **ELSE IF** the transition reports the same owner persisting, but its matched prefix changed (`prefix-changed`) - `inst-elseif-prefix-changed`
   1. [ ] - `p1` - The consumer remounts that owner through its own mount mechanism, scoped to the newly matched prefix as its `basepath` (§1.4, Binding obligation), rather than treating this as in-territory navigation the already-mounted instance will absorb - `inst-consumer-remounts-new-prefix`
5. [ ] - `p1` - **ELSE IF** the transition reports the same owner already mounted, with only the remainder changed - `inst-elseif-remainder-changed`
...
6. [ ] - `p1` - **ELSE** the transition reports no owner, or an owner disappearing - `inst-else-no-owner`
...
2. [ ] - `p1` - **WHEN** the navigation substrate's fan-out notifies the observer of a navigation - `inst-when-navigation`
   ...
   6. [ ] - `p1` - **ELSE IF** the previous and new owner are the same declared owner, and the new matched prefix differs from the previously recorded matched prefix - `inst-elseif-prefix-changed`
      1. [ ] - `p1` - Report a transition of kind `prefix-changed` — the same owner persists, but the declared prefix it matched under changed, because that owner declared more than one prefix and the URL now matches a different one of them - `inst-report-prefix-changed`
   7. [ ] - `p1` - **ELSE IF** the previous and new owner are the same declared owner, the new matched prefix equals the previously recorded matched prefix, and the current pathname differs from the previously recorded pathname - `inst-elseif-remainder-changed`
      1. [ ] - `p1` - Report a transition of kind `remainder-changed` — the pathname beneath the unchanged matched prefix moved without a change of owner or of matched prefix - `inst-report-remainder-changed`
   8. [ ] - `p1` - **ELSE IF** the previous and new owner are the same declared owner, the matched prefix is unchanged, and the current pathname equals the previously recorded pathname - `inst-elseif-same-owner-same-pathname`
   ...
   9. [ ] - `p1` - **ELSE** the previous and new owner are both "no owner" - `inst-else-no-owner-to-no-owner`
   ...
   10. [ ] - `p1` - Record the just-resolved owner, matched prefix, and current pathname as the observer's new previous state, regardless of which branch above ran - `inst-record-navigation-state`
3. [ ] - `p1` - **WHEN** the owner-prefix pairs source's own change notification fires, for a source that exposes one (§1.5) - `inst-when-pairs-source-changes`
   1. [ ] - `p1` - Re-run steps 2.1 through 2.10 against the current pathname and the source's now-current pairs, exactly as if a navigation had occurred — a registration or deregistration can change which owner a fixed pathname resolves to without the URL itself moving - `inst-reresolve-on-pairs-change`

**Rationale**: Distinguishing `prefix-changed` from `remainder-changed` is what lets a consumer tell a change of *which* prefix an owner matched under from a change confined *beneath* an unchanged one, without this feature inventing that distinction after the fact: an owner that declares more than one prefix has one mounted instance scoped to whichever prefix produced the mount, and only the consumer's own mount mechanism knows that scoping — this feature merely reports which of the two happened so the consumer does not have to re-derive it by comparing prefixes itself. Tracking the previously recorded matched prefix, not only the previously recorded owner, is what makes that distinction possible in the first place. Reporting a `remainder-changed` transition rather than staying silent when neither the owner nor its matched prefix changes is what lets a consumer whose own mount mechanism cares about in-territory navigation — for example, to keep an outer layout in sync — react without this feature inventing knowledge of what that reaction should be; the consumer's own mechanism decides what, if anything, to do with it. Tracking the previously recorded pathname, not only the previously recorded owner and prefix, is what makes that reporting precise: without it, a navigation that leaves the pathname exactly as it was — most notably one that changes only the current URL's search or hash — would be indistinguishable from one that actually moved within the owner's territory, and would report a `remainder-changed` transition carrying no real change for the consumer to act on. Filtering out a "no owner" to "no owner" transition, and a same-owner/same-prefix/same-pathname transition, are the two cases this algorithm treats as not ownership-relevant: nothing about occupancy could have changed when no declared prefix matched before or after, or when the pathname beneath a persisting owner's unchanged matched prefix is unchanged. Reporting an initial transition at creation, rather than only from the first subsequent navigation, is what covers a cold load or a reload — the very case a consumer most needs this signal for — from the first moment the observer exists.
...
- [ ] On every subsequent ownership-relevant navigation, the observer reports exactly one transition: `appeared` when the owner goes from none to a declared owner, `disappeared` when it goes from a declared owner to none, `changed` when it moves from one declared owner to a different one, `prefix-changed` when the same declared owner remains but the matched prefix itself changed (the owner declared more than one prefix and the URL now matches a different one), and `remainder-changed` when the same declared owner and the same matched prefix remain but the pathname beneath that prefix moved.
- [ ] No transition is reported when the resolved owner is "no owner" both before and after a navigation, nor when the same owner and the same matched prefix persist with the pathname beneath it unchanged; a navigation that changes only the current URL's search or hash falls into this latter case and produces no transition, because resolution matches on pathname alone.
- [ ] A `prefix-changed` transition carries both the previous matched prefix and the new one, so a consumer can tell which prefix its already-mounted instance was scoped to and which prefix it must remount under; `prefix-changed` and `remainder-changed` never both apply to the same transition.
