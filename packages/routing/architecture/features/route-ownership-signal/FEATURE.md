...
- A level's own base is handed to it by its enclosing level at creation, and that same enclosing level owns re-handing it: this feature's own observer never updates a base in place (§3, Observable Owner-Change Signal — Base-mismatch note). When the enclosing level's own engine re-matches the parametric value a nested level's base was built from to a different value, the enclosing level's own consumer releases the observer holding the stale base and creates a fresh one carrying the new value — release and recreate, never a protocol for updating the base of an observer still running, because a base is immutable by construction for the lifetime of the observer it was handed to.
...
3. [ ] - `p1` - Keep the portion of that value up to and including the given base, discarding whatever the carrier held deeper than the base - `inst-keep-up-to-base`
4. [ ] - `p1` - Substitute the given declared prefix as the remainder beneath the kept base, in place of whatever remainder the carrier previously held there - `inst-substitute-remainder`
5. [ ] - `p1` - **IF** the caller supplied a search and/or a hash to carry forward (meaningful only when this level's own carrier is the pathname) - `inst-if-caller-supplied-carryover`
   1. [ ] - `p1` - Read exactly the value the caller supplied for each - `inst-read-caller-supplied-carryover`
6. [ ] - `p1` - **ELSE** the caller supplied neither - `inst-else-no-carryover`
...
7. [ ] - `p1` - Call the shared history's `replace` — never `push` — writing the value composed in steps 3–4 back into this level's own carrier (the pathname, or the named parallel-axis key's own entry, whichever carrier this level has), together with whatever search and hash resulted from step 5 or step 6 when the carrier is the pathname - `inst-call-replace`
