...
- **Parse result** — the shell subroute and the hash, each copied verbatim from the input and returned untouched; an ordered list of entries, each carrying its own `domainKey`, its own `extension`, and its own ordered list of `params` (`{name, value}` pairs, in the order encountered, later occurrences of a duplicate name overwriting the value in place); an ordered list of **foreign segments** — the raw text of every `&`-delimited query segment that did not become an entry because it does not parse as one at all, in original relative order among themselves (below); and an ordered list of warnings, each naming the raw text of the entry it concerns and which of the parse-time edge rules it was produced by (malformed entry, duplicate parameter, duplicate extension). A well-formed input with zero entries parses to an empty entry list, not an error.
- **Foreign segments** — a query segment this package never treats as its own business: no `=` between a candidate domain key and extension, or a candidate domain key failing the `domain-key` production. Such a segment produces no warning, because it is not a malformed attempt at an entry, only a value some other party put in the same query string (an OAuth callback's `state`, an analytics `utm_*` parameter) — kept verbatim rather than the earlier, incidental behaviour of surviving only when it happened to also satisfy the entry grammar by coincidence. A segment that *does* satisfy the `domain-key` production but still fails to become an entry — an invalid extension token, or a param breaking the entry rules further in — warns `malformed-entry` only when its own shape is unmistakably an entry of this grammar: the domain key already carries the ancestry form (a `.`-composed key), or the segment already carries `;` parameters. A bare, single-segment `key=value` with an invalid extension and no `;` parameters is exactly as shaped as a foreign query parameter — this package has no registered-domain list to tell the two apart by name — so it is collected as a foreign segment silently, on the same terms as a plainly-foreign one, rather than warning on every parse of a value it can never confirm is its own. A segment dropped only for colliding with an earlier one under the same domain key (`duplicate-extension`) is neither: it *did* parse as an entry, so it is not collected here.
- **Serialize input** — the identical shape: a shell subroute, a hash, an ordered entry list of `{domainKey, extension, params}`, and the ordered foreign-segments list. Serializing is this parse result's own inverse for every entry and every foreign segment the parser actually kept, for canonical input only — never fed a raw parse warning, only what survived it. A bare `param-name` with no `=` and a `param-name=` with an explicit, empty value both parse to the identical empty-string value (§9 of the URL grammar's own edge rules), and the serializer always re-emits that value as a bare name; a parse→serialize round-trip is therefore byte-exact only when the input already used the canonical bare form for every empty value, not when it used the explicit `k=` form. Foreign segments re-emit after every entry, in the entries' own canonical order, then in the foreign segments' own original relative order among themselves — not necessarily interleaved with entries the way the source query string had them, since this package does not track that original interleaving once the two lists are separated (ADR 0003, "Repetition and order"); a query string carrying only foreign segments (zero entries) round-trips byte-exactly under this rule, since there is nothing else to place them after. This feature does not claim "own inverse" unqualified.
...
- [x] `p2` - **ID**: `cpt-frontx-algo-routing-navigation-substrate-position-tracking`
...
**Steps**:
1. [x] - `p1` - At construction, read this instance's own position back out of the current entry's own browser-persisted state, under this package's own namespaced key, defaulting to `0` when none is present — a cold mount (the page's very first entry, never written by this instance) or a "foreign" entry (one a real back/forward step or a third-party addition landed on, that this instance never itself wrote a position onto); nothing is written back to the browser's own state for this default yet — recorded for real only the first time this instance's own `push` or `replace` runs (step 2) - `inst-cold-mount-default`
2. [x] - `p1` - **WHEN** this instance's own `push` or `replace` is called (§3, Fan-Out Subscription Dispatch, step 3) - `inst-on-own-write`
   1. [x] - `p1` - **IF** the call is `push` — advance this instance's own position by one over its own previous value, and record that new position under this package's own namespaced key on the *new* entry's own state — never carrying the previous entry's own state forward onto it, exactly as a real `pushState` call never does on its own - `inst-advance-on-push`
   2. [x] - `p1` - **ELSE** the call is `replace` — leave this instance's own position exactly as it already is, and record it under this package's own namespaced key merged into whatever raw state the browser already carries on the *current* entry — every other key already present on that entry's own state survives this write untouched - `inst-preserve-on-replace`
...
**Output**: The shell subroute and the hash, each copied verbatim from the input; an ordered list of entries, each `{domainKey, extension, params}` with `params` itself an ordered list of `{name, value}` pairs; an ordered list of foreign segments (§1.5, "Foreign segments"); and an ordered list of warnings, each naming the raw entry text it concerns and which rule produced it. Parsing never throws — a malformed entry is dropped and reported, every other entry is kept (`cpt-frontx-routing-adr-domain-occupancy-addressing-granularity`).
...
3. [x] - `p1` - **IF** the query string is empty — including a URL whose `?` is present with nothing after it (e.g. `/en?`), which is a present, empty query string, not the absence of one - `inst-if-empty-query`
   1. [x] - `p1` - **RETURN** an empty entry list, an empty foreign-segments list, no warnings, and the copied shell subroute and hash - `inst-return-empty`
...
5. [x] - `p1` - **FOR EACH** raw entry string, in order - `inst-foreach-raw-entry`
   0. [x] - `p1` - **IF** this raw entry string is empty — produced by two consecutive `&` characters (`a=b&&c=d`) or a trailing `&` at the end of the query string — silently skip it and continue to the next raw entry string: no warning is recorded for an empty raw entry, unlike every other malformed case this algorithm reports, and it is not collected as a foreign segment either — there is no text to keep - `inst-skip-empty-raw-entry`
   1. [x] - `p1` - Split it on `;` into its head segment and zero or more param segments - `inst-split-on-semicolon`
   2. [x] - `p1` - **IF** the head segment carries no `=`, or the text before that `=` (the candidate domain key) does not conform to the `domain-key` production — an odd count of `.`-separated segments, each segment matching the `name` alphabet - `inst-if-plainly-foreign`
      1. [x] - `p1` - This raw entry string is plainly not an entry at all, and is not this package's own business (ADR 0003, "Repetition and order"): append it to the output foreign-segments list, unchanged and with no warning, and continue to the next raw entry string - `inst-keep-foreign`
   3. [x] - `p1` - **IF** the text after the head's `=` (the candidate extension) does not match the `name` alphabet - `inst-if-malformed-head`
      0. [x] - `p1` - Decide whether this raw entry string is an **entry candidate** — its shape unmistakably that of an entry of this grammar, rather than an unrelated query parameter that merely happens to have a `name`-shaped key — because its candidate domain key already carries the ancestry form (contains a `.`), or this raw entry string's own param segments (step 5.1) are non-empty (a `;` is present). This package is never given the caller's own registered domain names to decide by name instead — its input type carries only the shell subroute, query string, and hash, nowhere else in this algorithm or any of its call sites - `inst-entry-candidate-shape`
      1. [x] - `p1` - This raw entry string has a valid-looking domain key but is still not a well-formed entry: drop it and, **only if** it is an entry candidate (the previous step), record a warning citing its raw text and "malformed entry" — a bare, single-segment domain key with no `;` parameters is exactly as shaped as a foreign query parameter and stays silent no matter how invalid its extension looks. Either way, *also* append it to the output foreign-segments list (on the same terms as step 5.2.1 above — this package still keeps the text verbatim), and continue to the next raw entry string - `inst-drop-malformed`
   4. [x] - `p1` - Set `domainKey` and `extension` from the head segment's own two sides of its first `=` - `inst-set-domain-key-extension`
   5. [x] - `p1` - **IF** `extension` already appears in an entry already kept for this same `domainKey` earlier in this same parse - `inst-if-duplicate-extension`
      1. [x] - `p1` - Drop this entry (the first occurrence already kept stands), record a warning citing its raw text and "duplicate extension", and continue to the next raw entry string — this one *did* parse as a well-formed entry, so it is not collected as a foreign segment; it lost only to a collision rule this package enforces on well-formed entries - `inst-drop-duplicate-extension`
   6. [x] - `p1` - **FOR EACH** param segment, in order - `inst-foreach-param-segment`
      ...
      4. [x] - `p1` - **IF** a `%` in this segment is not followed by two hexadecimal digits (a malformed escape, e.g. `%zz` or a trailing `%`), or the bytes a run of `%XX` escapes assembles are not valid UTF-8 - `inst-if-malformed-escape`
         1. [x] - `p1` - Drop the *whole entry* this segment belongs to — not only this one param — record a warning citing the entry's raw text and "malformed entry", *also* append it to the output foreign-segments list (same terms as step 5.3.1), and continue to the next raw entry string, abandoning whatever params of this entry were already collected - `inst-drop-entry-malformed-escape`
   ...
   7. [x] - `p1` - Append `{domainKey, extension, params}` to the output entry list, in this raw entry string's own position - `inst-append-entry`
6. [x] - `p1` - **RETURN** the entry list, the foreign-segments list, the warnings collected, and the copied shell subroute and hash - `inst-return-parsed`

**Rationale**: Malformed-entry, duplicate-extension, and duplicate-parameter are three independent edge rules the grammar deliberately resolves differently — a malformed entry is discarded outright because there is no well-formed fact to keep as an *entry* (though its raw text still survives as a foreign segment, below); a duplicate extension under one domain key keeps the first occurrence because entries are an ordered list a domain reads meaning from; a duplicate parameter name keeps the last occurrence because parameters are a map, the same rule `URLSearchParams` itself would apply if this grammar's own delimiters let it be used at all. Every rule reports a warning rather than throwing, because a consumer resolving an existing, possibly bookmarked URL cannot recover from a thrown parse error the way it can from a dropped or overwritten entry. Foreign segments exist because a query string is shared ground: a segment this package cannot make an entry out of is not, by that fact alone, wrong — it may simply belong to another party (an OAuth callback's `state`, an analytics `utm_*` parameter) — so step 5.2 keeps it verbatim rather than the earlier behaviour of losing it, or keeping it only by the accident of its also happening to satisfy the entry grammar. Step 5.3's warning fires only for an entry candidate (step 5.3.0) — a domain key already in the ancestry form, or a segment already carrying `;` parameters — because either shape rules out an unrelated party's own parameter; a bare, single-segment `key=value` with an invalid extension is exactly as plausible as a foreign one, and this algorithm has no registered-domain list to break the tie by name, so it stays silent rather than warning on every parse of a value it can never confirm is actually this grammar's own.

### Grammar Serialize

- [x] `p2` - **ID**: `cpt-frontx-algo-routing-navigation-substrate-grammar-serialize`

**Input**: A shell subroute, an ordered entry list of `{domainKey, extension, params}` (`params` itself ordered), an ordered foreign-segments list, and a hash — the identical shape `cpt-frontx-algo-routing-navigation-substrate-grammar-parse` produces. The foreign-segments list is optional on this algorithm's own accepted input, unlike on the parse result it otherwise mirrors: a caller building the input by hand for a URL with nothing foreign in it may omit it, and this algorithm treats an omitted list exactly as an empty one.

**Output**: A URL string, or a thrown error naming the offending entry (or, for step 0, the offending shell subroute, or, for step 1, the offending foreign segment) when the input violates a structural invariant this package itself enforces on write.

**Steps**:
0. [x] - `p1` - **IF** the given shell subroute contains `?`, `#`, or `&` - `inst-if-invalid-shell-subroute`
   1. [x] - `p1` - **THROW** an error naming the offending shell subroute — a shell subroute produced by grammar parse can never contain one of these three characters (parse cuts it off at the first `?`), so this rejects only a caller-constructed `SerializeInput` that would otherwise reparse into a different, silently corrupted structure the moment the written URL is read back - `inst-throw-invalid-shell-subroute`
1. [x] - `p1` - **FOR EACH** foreign segment in the input list - `inst-foreach-foreign-segment-validate`
   1. [x] - `p1` - **IF** it contains `"&"` or `"#"` - `inst-if-invalid-foreign-segment`
      1. [x] - `p1` - **THROW** an error naming the offending foreign segment — a foreign segment produced by grammar parse can never contain either character (parse only ever collects a segment already split on `"&"`, with any `"#"` already cut off into the hash), so this rejects only a caller-constructed `SerializeInput` that would otherwise reparse into a different, silently corrupted structure the moment the written URL is read back: an embedded `"&"` splits it into an entry-or-foreign-segment position it never had; an embedded `"#"` moves everything after it out of the query string and into the fragment - `inst-throw-invalid-foreign-segment`
2. [x] - `p1` - **FOR EACH** entry in the input list - `inst-foreach-entry-validate`
   ...
   2. [x] - `p1` - **IF** this entry's own `params` list carries a param whose own `name` is the empty string — `param-name` requires at least one character, unlike `param-value`, which the identical production allows empty (ADR 0003, "Tokens") - `inst-if-invalid-param-name`
      1. [x] - `p1` - **THROW** an error naming this entry — an unvalidated empty name would serialize to a bare `;=value` segment that reparses as a malformed entry, dropped whole, the instant the written URL is read back: the third case of the same reparse-hazard family steps 0 and 1 above already guard - `inst-throw-invalid-param-name`
   3. [x] - `p1` - **IF** this entry's own `params` list carries two params of the identical `name` - `inst-if-duplicate-param-name`
      1. [x] - `p1` - **THROW** an error naming this entry - `inst-throw-duplicate-param`
   4. [x] - `p1` - **IF** an entry earlier in this same input list already carries the identical `domainKey` and `extension` - `inst-if-duplicate-extension-serialize`
...
3. [x] - `p1` - **FOR EACH** entry, in the input list's own order - `inst-foreach-entry-build`
...
4. [x] - `p1` - Join the built entry texts with `"&"` - `inst-join-entries`
   1. [x] - `p1` - Append every foreign segment from the input (§1.5, "Foreign segments"), in its own original relative order, after the joined entry texts, each separated by `"&"` — foreign segments are never interleaved back among entries the way the source query string may have had them, since parse already separated the two lists and this algorithm does not track their original interleaving - `inst-append-foreign-segments`
5. [x] - `p1` - **IF** the input entry list and the input foreign-segments list are both empty - `inst-if-zero-entries`
   1. [x] - `p1` - **RETURN** the shell subroute alone, followed by the hash if one is present, with no `"?"` at all - `inst-return-bare-subroute`
6. [x] - `p1` - **ELSE** - `inst-else-nonzero-entries`
   1. [x] - `p1` - **RETURN** the shell subroute, `"?"`, the joined entry texts and foreign segments from step 4, and the hash if one is present - `inst-return-full-url`

**Rationale**: Serialize enforces at write time exactly the invariants parse tolerates at read time, and for the opposite reason: a caller building an entry list controls every entry in it, so a duplicate extension, a duplicate parameter name, an empty parameter name, or a malformed token is a programming error to surface immediately rather than a stray input to recover from. An empty parameter *value*, by contrast, is not rejected: `param-value` is `*( pchar-safe | pct-encoded )` (ADR 0003, "Tokens"), zero or more, unlike `param-name`'s own `1*(...)` — a bare `name` with no `=` is already the canonical, complete encoding of an empty-valued param (step 3.2.3), so nothing about step 2.2 extends to it. The same reasoning motivates step 0's shell-subroute check and step 1's foreign-segment check: a caller building a `SerializeInput` directly, never routed through parse, controls those values too, so a `?`/`#`/`&` inside the shell subroute, or an `&`/`#` inside a foreign segment, is equally a programming error to surface immediately rather than a corrupted URL to write silently. The zero-entries-and-zero-foreign-segments case emits no trailing `?` (ADR 0003's own example 7.8) because a bare shell subroute is a valid, fully resolved state — every domain at zero occupants and nothing foreign in the query string either — not a partial or error state calling for a placeholder query string; a query string carrying only foreign segments still needs the `?`, because there is content to write even though no domain projects an entry.
...
- [x] Parsing a query string in which every domain is at zero occupants — an empty query string — and serializing the resulting empty entry list back reproduces the bare shell subroute alone, with no trailing `?`:
  ...
  /en
  ```
- [x] A raw entry that is plainly not an entry at all — no `=` between a candidate domain key and extension, or a candidate domain key with a token outside the `name` alphabet or an even segment count, e.g. `/en?screen&a.b=x&widgets=line-a;range=7d` — is kept as a foreign segment, verbatim, with no warning: here, `screen` has no `=` at all, and `a.b=x` is its own segment whose candidate domain key `a.b` has an even segment count (two), so both are foreign, while `widgets=line-a;range=7d` is kept as an entry and parsing does not throw.
- [x] A raw entry whose candidate domain key does conform, but whose extension does not match the `name` alphabet, warns `malformed-entry` only when the raw entry string is an entry candidate (step 5.3.0): `screen.app.panel=Bad_1` (its domain key carries the ancestry form) and `screen=Bad_1;tab=x` (its own param segments are non-empty) both drop from the parsed entry list with a warning citing their own raw text and "malformed entry" — they are also kept as a foreign segment, on the same terms as one. A single-segment domain key with no `;` parameters, e.g. `page=2`, `utm_source=news`, `code=4%2F0AX`, a base64-looking `state=…`, or a bare `flag`, is kept as a foreign segment with no warning at all, however invalid its extension looks, because nothing in its shape rules out an unrelated query parameter — and it survives a write unchanged. A raw entry dropped only for colliding with an earlier one under the same domain key (`duplicate-extension`) is neither warned as malformed nor kept foreign either way, because it did parse as a well-formed entry.
- [x] A write that touches only its own domain key's entries never erases a foreign query parameter another party put in the same query string: parsing `/en?screen=app&utm_source=news&code=oauth123` and re-serializing after changing only `screen`'s own params keeps both `utm_source=news` and `code=oauth123` in the resulting URL; an OAuth-style return, `?code=abc123&state=xyz789`, survives an unrelated write identically; and a query string carrying only foreign segments (zero entries) round-trips through parse then serialize byte-exactly.
- [x] `serializeGrammar` throws `invalid-shell-subroute` for a shell subroute containing `?`, `#`, or `&` instead of silently producing a URL that reparses to a different, corrupted entry list — a normal shell subroute is unaffected.
- [x] `serializeGrammar` throws `invalid-foreign-segment` for a hand-built foreign segment containing `&` (e.g. `x&screen=evil`) or `#` (e.g. `x#frag`), instead of silently writing a URL that reparses with an injected entry or with text bled into the fragment — a normal foreign segment carrying neither character is unaffected.
- [x] `serializeGrammar` accepts a `SerializeInput` whose `foreignSegments` field is omitted entirely and serializes it exactly as it would an explicit empty list, emitting only the entries.
