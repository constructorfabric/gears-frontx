...
2. [ ] - `p1` - **FOR EACH** declared prefix that matches the pathname at a segment boundary — the pathname equals the declared prefix exactly, or the pathname continues immediately after the declared prefix with the path-segment separator (`/`) — a match on a partial segment does not qualify: a declared prefix of `/user` is not a match for a pathname of `/users/42`, because the character immediately following `/user` there is `s`, not `/` - `inst-foreach-candidate`
...
**Root-prefix note**: A declared prefix of `/` (or the empty string, both denoting the application root) matches every pathname, including `/` itself — the segment-boundary rule in step 2 already holds for it, since every pathname either equals it (the empty string, or `/`) or continues with a `/` that is already there by construction. It is not a special case of the matching rule, only a candidate whose reach happens to be everything.
...
- [ ] Given a pathname and a set of declared identifier-to-prefix pairs, the resolution primitive returns the identifier whose declared prefix is the longest match at a path-segment boundary, and a "no owner" result when no declared prefix matches; a declared prefix of `/user` does not match a pathname of `/users/42`, and a declared prefix of `/` (or the empty string, denoting the application root) matches every pathname.
