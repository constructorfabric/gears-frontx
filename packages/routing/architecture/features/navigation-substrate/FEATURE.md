...
- **Navigation kind** — which of three kinds of navigation triggered this round: an added history entry (`push`), a replaced history entry (`replace`), or a move through existing history entries (`go` issued through this instance, a user's own back/forward step, or a third-party call to the browser's own history API that itself moves through existing entries, e.g. `history.go`). This third case is the only third-party history mutation this substrate observes at all: a third-party call that adds or replaces an entry directly (`pushState`/`replaceState` invoked outside this instance) raises no event this substrate's underlying subscription listens for, so it produces no notification and is not classified as anything (PRD §3.1; DESIGN §4 failure modes). This is `NavigationHistory`'s own classification, carried in every notification regardless of which of the two dispatch paths (§3, Fan-Out Subscription Dispatch) produced it.

This shape is `NavigationHistory`'s own notification, internal to this contract until an adapter translates it into a different contract's shape — the Engine Provider is that adapter for whatever contract shape its own concrete engine's `subscribe` callback expects (`cpt-frontx-feature-routing-engine-provider` §3, History Adaptation): it derives that shape's own action/kind field from this shape's navigation-kind field and its own location field from this shape's Location field, rather than inventing either. A caller who subscribes directly against `NavigationHistory`, without an adapter in between, receives this shape as-is.
...
- **`replace(path)`** — accepts a path string in the same shape as `push` and overwrites the current history entry with it, leaving every entry before and after the current one untouched.
...
- The `basepath` assigned to the microfrontend it is mounted for — host-assigned when the microfrontend is composed, deployment-supplied or absent when it is served standalone.
...
**Input**: A pathname; the set of declared identifier-to-prefix pairs, supplied by the caller as a plain argument.

**Output**: The identifier of the route owner whose declared prefix is the longest match for the pathname, together with that specific matched declared prefix — an owner may declare more than one prefix, so the matched prefix is never implied by the identifier alone — or a "no owner" result when no declared prefix matches.
