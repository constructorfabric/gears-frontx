...
- **Reference integration**: `template-mfe`'s demo-mfe screens and its widgets fixture in `constructorfabric/gears-frontx-templates` are the reference consumers of this provider, composed under `template-shell`: each occupant reads its entry address from the template's entry-addresses shared property and passes it to `adaptProviderHistory`. That code is template territory and carries no authoritative markers (`cpt-frontx-adr-template-territory-traceability`); this feature's consumer-side instructions stay unticked.
