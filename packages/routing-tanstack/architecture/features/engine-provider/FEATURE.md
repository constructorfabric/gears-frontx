...
- **Reference integration**: `template-mfe`'s demo-mfe screens and its widgets fixture in `constructorfabric/gears-frontx-templates` are designated as the reference consumers of this provider, composed under `template-shell` (lands with the templates PR for #638): each occupant is to read its entry address from the template's entry-addresses shared property and pass it to `adaptProviderHistory`. That code is template territory and carries no authoritative markers (`cpt-frontx-adr-template-territory-traceability`); this feature's consumer-side instructions stay unticked.
...
- [x] `p2` - **ID**: `cpt-frontx-algo-routing-engine-provider-history-adaptation`
...
5. [x] - `p1` - Construct `block` as a recognized, degraded adaptation rather than a member routinely derived like steps 3 and 4's: `NavigationHistory`'s fan-out notifies a subscriber only after a navigation has already taken effect (`cpt-frontx-algo-routing-navigation-substrate-fanout-dispatch`), so nothing in the substrate's own contract lets this adapter intercept and cancel a navigation before it happens. The constructed `block` can stop only a navigation issued through this same constructed router's own `RouterHistory` object — a call the engine routes back through this adapter before it ever reaches the shared history; it cannot stop a back/forward step, a `go` call issued by another unit, or a navigation any other unit performs directly through the shared `NavigationHistory` — the shared history commits any of those regardless of a block state this adapter holds. A registered blocker that throws or rejects is treated as blocking the navigation it was asked about — the safest reading of "the blocker could not decide" — and the failure is reported through this adapter's own error-reporting channel (`AdaptHistoryOptions.reportError` — a consumer's own callback when given, `console.error` otherwise — accepted by `adaptVirtualLocationHistory` and forwarded unchanged by `adaptComposedHistory`, `adaptStandaloneHistory`, and `adaptProviderHistory`, every entry point that builds this adapter's own history) rather than left to surface as an unhandled promise rejection - `inst-derive-block-degraded`
6. [x] - `p1` - Accept and discard the per-entry state argument `RouterHistory`'s own `push` and `replace` carry, as a second recognized, degraded adaptation alongside step 5's: the navigation substrate excludes caller-owned per-entry state from its contract outright — `push` and `replace` take a path alone, its `Location` shape carries no field for one, and it owns the browser's own per-entry state exclusively under a single key of its own, rewriting it on every write (`cpt-frontx-feature-routing-navigation-substrate` §1.5, "Entry-carried state — not part of this contract"). This adapter therefore never stores, forwards, or returns that argument, and **MUST NOT** substitute a provider-local map for it: such a map is emptied by the first reload and bypassed by the first back step, so it would promise a durability this provider cannot deliver. The consequence is stated rather than hidden: `useLocation().state` and `navigate({ state })` never carry a consumer's own value, and route masking — which the engine builds on that same per-entry state — does not work. The one member of the state object this adapter does populate is the engine's own index field, derived from the substrate's own `Location.position` (step 3) and never from a caller. A consumer needing entry-carried state gets it from a different engine-provider port implementation over an engine whose own contract provides it, never from this one - `inst-accept-discard-entry-state`
7. [x] - `p1` - Expose `RouterHistory`'s `subscribe(cb)` by registering an internal callback against `NavigationHistory`'s own `subscribe`, re-projecting this occupant's own entry into a virtual location on every notification, and from inside that internal callback, construct the `SubscriberArgs` shape (`location`, `action`) `RouterHistory`'s `cb` expects before invoking `cb` with it: `SubscriberArgs.location` is the re-projected virtual location, and `SubscriberArgs.action` is derived directly from `NavigationHistory`'s own notification's navigation-kind field (`cpt-frontx-feature-routing-navigation-substrate` §1.5, Contract Shapes — `push`, `replace`, or a third kind covering both a history move and an observed third-party addition such as fragment navigation) — `action` is never invented or independently inferred by this adapter. Fan-out to this constructed `RouterHistory`'s own external subscribers is dispatched over a snapshot of the subscriber registry, with each individual subscriber's own invocation isolated in its own error boundary, mirroring the navigation substrate's own fan-out dispatcher (`cpt-frontx-algo-routing-navigation-substrate-fanout-dispatch`): a throwing subscriber's own failure is reported, never left to stop delivery to the subscribers registered after it in the same round - `inst-adapt-subscribe`
8. [x] - `p1` - **IF** this occupant's own entry is absent from the newly parsed entry list at the moment step 7's own re-projection runs — removed by a back step, or by a structural reset, while this occupant has not yet been unmounted - `inst-if-own-entry-absent`
   1. [x] - `p1` - The virtual location keeps its own last-projected value rather than re-projecting to some default; no write-back runs, because there is no longer an entry of this occupant's own to write to; and `cb` is not invoked for this occupant's own `RouterHistory` — the constructed router receives no navigation of its own from this round. Teardown (§3, Teardown On Unmount) still runs normally once whichever consumer's mount mechanism actually unmounts this occupant, in response to the same removal - `inst-own-entry-absent-inert`
9. [x] - `p1` - **RETURN** the adapted `RouterHistory` object - `inst-return-adapted-history`
...
- [x] `p2` - **ID**: `cpt-frontx-algo-routing-engine-provider-router-creation`
...
**Steps**:
1. [x] - `p1` - Call `createRouter({ routeTree, history })` with the microfrontend's own route tree and the adapted, virtual history, and admit alongside them every other construction option the engine accepts, supplied by the consumer and passed through untouched — `context` above all, the seam an application's route loaders reach a consumer-owned dependency such as an API client through. `routeTree` and `history` are written last, so no supplied option can displace either. This seam lives on this provider's own exports — a third argument to its construction function and a prop on its mount component — and **MUST NOT** be added to the engine-provider port: the port's own input is exactly `{history, entryAddress, routeTree}` (`cpt-frontx-feature-routing-navigation-substrate` §1.5, Engine-provider port shape), and widening it with an engine-shaped options field would put a concrete engine's construction surface in the core package that declares it. The port-typed entry point therefore supplies no options at all, and the router it returns carries no context - `inst-call-create-router`
...
- [x] `p2` - **ID**: `cpt-frontx-algo-routing-engine-provider-index-redirect`
...
**Steps**:
1. [x] - `p1` - Accept the target path from the caller — a route's own redirect target, an index route's resolved destination, or any other consumer-supplied path — interpreted against this occupant's own virtual location, never against the composed-application URL directly - `inst-accept-target-path`
2. [x] - `p1` - Read the current virtual location's search, and the page's own current hash - `inst-read-current-search-hash`
3. [x] - `p1` - Carry that search and that hash forward onto the target path, rather than requiring the caller to assemble the carry-forward itself - `inst-carry-search-hash`
4. [x] - `p1` - **RETURN** the `redirect` (or navigation call) to the target path with search and hash intact, to be written back to this occupant's own entry alone through the navigation substrate's own control boundary — the search into that entry's own payload, the hash exactly as read, since this package never interprets it - `inst-return-redirect`
...
- [x] `p2` - **ID**: `cpt-frontx-algo-routing-engine-provider-teardown`

**Input**: A constructed router being mounted into, or unmounted from, a microfrontend's own component tree; the adapted `RouterHistory` that router carries, and the internal callback (§3, History Adaptation) that history registers against the shared `NavigationHistory`'s `subscribe`.

**Output**: The internal callback registered against the shared `NavigationHistory` for exactly as long as the router is mounted, and unsubscribed once it is not, so an unmounted router's own `RouterHistory` stops receiving the fan-out.

**Steps**:
1. [x] - `p1` - **WHEN** the microfrontend that owns this constructed router mounts it (the same level's own consumer that will later unmount it) - `inst-when-mount`
   1. [x] - `p1` - Register this adapter's internal callback against the shared `NavigationHistory`'s `subscribe`, keeping the unsubscribe function that call returns. This registration is established here and **MUST NOT** be established when the adapted history is constructed: an adapted history's lifetime is one occupancy of a domain, and construction carries no promise that the object will ever be mounted, so registering there binds a realm-lived resource to an act that may be discarded — a history built and dropped before any mount keeps a live registration with nothing left holding a reference to release it. Constructing an adapted history therefore leaves it inert: its virtual location holds what it projected at construction and no navigation from elsewhere reaches it until this step runs. A consumer that mounts a constructed router without this provider's own mount component owns running this step itself, through the attach function this package publishes, exactly as it already owns the matching teardown below. Re-establishing a registration that is already active is a no-op, which is what makes this step and step 2 below exact inverses under a mount mechanism that runs setup, cleanup and setup again within one mount - `inst-register-internal-callback`
2. [x] - `p1` - **WHEN** the microfrontend that owns this constructed router is unmounted (whichever level's own consumer mounted it — the host's, at the outermost domain, or an enclosing extension's, at any level nested deeper — unmounting it, informed by `cpt-frontx-feature-routing-route-ownership-signal`'s observable ownership-change transition, or that same consumer's own teardown for any other reason) - `inst-when-unmount`
   1. [x] - `p1` - Invoke the unsubscribe function returned when this adapter registered its internal callback against the shared `NavigationHistory`'s `subscribe` - `inst-invoke-unsubscribe`
3. [x] - `p1` - **RETURN** - `inst-return-teardown`

**Rationale**: Pairing the registration with the mount rather than with construction is what makes a discarded instance free: a mount mechanism that builds a candidate history and then drops it — React's StrictMode double-invoking a memo that constructs one is the ordinary case — would otherwise leave a live registration against a shared history that outlives the page's interest in it, with no reference left to release it through. If the release half is skipped, the torn-down router's own `RouterHistory` callback keeps receiving the shared history's fan-out for a component tree that no longer exists — a leaked subscription that grows with every mount/unmount cycle a long-lived host session runs through. This is the Engine Provider's own responsibility because it is the component that registered the internal callback in the first place (§3, History Adaptation); the navigation substrate has no notion of "this listener's microfrontend was unmounted" to act on even if it wanted to. The router and its `RouterHistory`'s own subscribers are scoped to that registration, not to any one mount of the Engine Provider: this teardown step releases only the internal callback's registration against the shared `NavigationHistory`, leaving the constructed router and its `RouterHistory`'s own subscribers otherwise untouched, so mounting that same router again re-establishes the internal registration and resumes the fan-out to whichever subscribers stayed registered through the gap — the same remount-survives-by-default property the navigation substrate already grants a bridge's own registrations and subscriptions (`cpt-frontx-adr-child-mfe-host-access`).
...
1. [x] - `p1` - **IF** no entry address is supplied — the microfrontend is served standalone - `inst-if-standalone`
   1. [x] - `p1` - Project the virtual location directly onto the page's own current pathname and search — the identical `route`-parameter convention applied to the page's own address as a whole, rather than to one entry inside a larger composed-application URL: the page's own pathname is this occupant's entire virtual pathname, and the page's own search is its entire virtual search, with no `route` parameter to extract because there is no enclosing entry to extract it from - `inst-project-standalone-location`
   2. [x] - `p1` - Write-back in this mode goes directly through the page's own history — no entry, no grammar codec, and no back-projection helper are involved at all, because there is no composed-application URL and no domain key to target; `push`/`replace`/`go` call the page's own `history.pushState`/`replaceState`/`go` with the page's own pathname and search built from the virtual location directly - `inst-standalone-write-back`
...
4. [x] - `p1` - Construct the router through the same `createRouter({ routeTree, history })` call the composed case uses, over the standalone-projected virtual history from step 1 - `inst-construct-with-standalone-virtual-history`
...
- [x] `p1` - **ID**: `cpt-frontx-dod-routing-engine-provider-redirect-and-standalone`
...
- [x] `p1` - **ID**: `cpt-frontx-dod-routing-engine-provider-teardown`

The system **MUST** register a constructed router's adapted `RouterHistory` against the shared `NavigationHistory` when the microfrontend that owns that router mounts it, **MUST NOT** register it when that history is merely constructed, and **MUST** unsubscribe it when that microfrontend is unmounted — so a router discarded before it is ever mounted leaves nothing registered, and a torn-down router's callback stops receiving the fan-out.
...
- [x] The default engine provider projects this occupant's own entry into a virtual location — the reserved `route` parameter as pathname, defaulting to `/` when absent, and every other parameter as search — and adapts it, together with the navigation substrate's shared `NavigationHistory`, into a `RouterHistory` object: `location` exposed as the projected virtual location; `push`/`replace`/`go` deriving to write only this occupant's own entry back through the navigation substrate's own control boundary; the further members the engine's contract requires beyond those derived per `cpt-frontx-algo-routing-engine-provider-history-adaptation`, including `block` constructed with that algorithm's recognized degraded semantics rather than as a routine derivation; and `SubscriberArgs` (`location`, `action`) constructed for `RouterHistory`'s `subscribe` callback from `NavigationHistory`'s own notification — `action` derived from that notification's navigation-kind field, never invented by this adapter.
- [x] The constructed `block` stops only a navigation issued through the same constructed router's own `RouterHistory` object; it does not stop a back/forward step, a `go` call issued by another unit, or a navigation another unit performs directly through the shared `NavigationHistory` — this narrowed scope is documented, not silently assumed.
- [x] `createRouter({ routeTree, history })` is called with the adapted, virtual history and the microfrontend's own route tree, and the resulting router matches only its own virtual location.
- [x] A consumer can supply the engine's further construction options — `context` in particular, carrying a dependency its route loaders read — through this provider's own construction function and its own mount component, and those options reach the constructed router; neither `routeTree` nor `history` can be displaced by them, and the engine-provider port's own input stays `{history, entryAddress, routeTree}`, so the port-typed entry point constructs a router with no context.
- [x] The per-entry state argument `push`/`replace` accept is discarded rather than stored anywhere: a consumer's own value never reaches `useLocation().state` or survives a `navigate({ state })`, and route masking does not work — the recognized consequence of the substrate excluding entry-carried state from its contract, recorded rather than worked around with a provider-local map.
- [x] A write the shared history refuses — its bounded fan-out drain raising on a subscriber that navigates from every round it receives — is reported through this adapter's own error-reporting channel and leaves the virtual location unchanged, rather than escaping `push`/`replace` as an unobserved promise rejection.
- [x] The constructed router is mounted into the microfrontend's own component tree via `RouterProvider`, as a separate act, performed by `EngineProvider`, from constructing it.
- [x] `createHref` composes a full composed-application URL by calling the navigation substrate's own grammar serializer over the current entry list with this occupant's own entry replaced, never by concatenating the virtual location's own pathname and search as if they were a standalone URL.
...
- [x] No package in this ecosystem other than this one imports a concrete router engine or its packages directly; this package is the sole, deliberate exception (`cpt-frontx-constraint-routing-tanstack-sole-engine-import`).
- [x] The location-preserving navigation helper preserves the current virtual location's search, and the page's own hash, for any consumer redirect within the occupant's own entry, including but not limited to a redirect issued from an index route.
- [x] A microfrontend deployed on its own runs the same router construction, projecting the identical virtual location directly onto the page's own pathname and search, with no route-ownership-signal observer created for this deployment. An undeclared `route=` path inside the microfrontend's own route tree resolves to the engine's own `notFound` identically in composed and standalone deployment — never to whichever level's own consumer shows as a fallback for an entry the route ownership signal reports unresolved, a distinct level this provider never decides in either mode.
- [x] When a microfrontend is unmounted, its constructed router's adapted `RouterHistory` is unsubscribed from the shared `NavigationHistory`, so it stops receiving further fan-out.
- [x] Constructing an adapted history registers nothing: two histories built for one mount, only one of which is kept, leave exactly one live registration while mounted and none after unmount. A constructed history observes the shared history only once a mount boundary attaches it, and a consumer mounting a router outside this provider's own mount component owns that attach as well as the matching teardown.
- [x] Mode selection by presence of an entry address: given an entry address, the constructed history projects its virtual location from that entry's own payload; given none, it projects the identical convention onto the page's own pathname and search — the same route tree and the same adaptation logic run in both cases, differing only in where the projected pathname and search come from.
- [x] One navigation sequence performed against the constructed router produces the identical resulting virtual location — the same reserved `route` parameter and the same remaining search parameters — whether the microfrontend is running composed, with the virtual location backed by one entry, or standalone, with the virtual location backed by the page's own address.
- [x] The provider never writes to a sibling occupant's own entry, another domain's own entry, or the shell subroute, for any navigation the constructed router performs, in either mode.
- [x] An implementation **MUST** reproduce this provider's own Example 1 (`packages/routing-tanstack/architecture/PRD.md` §9) as one of its own acceptance scenarios (§1.5):
...
- [x] An implementation **MUST** reproduce this provider's own Example 2 (`packages/routing-tanstack/architecture/PRD.md` §9) as one of its own acceptance scenarios — the same microfrontend served standalone, virtual location projected onto the page's own address:
