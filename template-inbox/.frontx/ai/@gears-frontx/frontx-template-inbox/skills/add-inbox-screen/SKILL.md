...
3. **Compose from `@gears-frontx/ui-kit` and its semantic tokens.** Consult the
   ...
   look-alike of a kit component is a defect, not a style choice. Layout goes in
   `src/styles/workspace.module.css` on kit tokens; no raw colours, no CSS
   framework, no second stylesheet.
