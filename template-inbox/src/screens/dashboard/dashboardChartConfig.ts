 ...
 * from the same four-hue palette so a viewer learns "violet is open
 ...
 * `--primary`/`--info`/`--success`/`--warning` instead means the palette
 * resolves through the kit's own light/dark values automatically, the same
 * way every other token-driven color in this app does.
 */
 ...
 * line, radial), the C-reference convention this screen borrows (see the
 * dashboard spec). */
...
export const teamUtilizationChartConfig: ChartConfig = {
  value: { label: 'Utilization', color: DASHBOARD_PALETTE.amber },
};
