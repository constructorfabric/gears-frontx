 ...
 * never has to be told where the instance lives. `MailApiService` and
 * `DashboardApiService` are the second and third services the doc comment on
 * `InboxApiService` anticipated - a separate domain gets a separate service
 * rather than a field bolted onto this one.
 */
...
export function registerApiServices(): void {
  ...
  // a server. Delete these lines the day there is a server.
  apiRegistry.getService(InboxApiService).useMocks(true);
  apiRegistry.getService(MailApiService).useMocks(true);
  apiRegistry.getService(DashboardApiService).useMocks(true);
}
