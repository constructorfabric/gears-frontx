...
export const messages: Message[] = [
  ...
  /*
   * c-11 - Liam Chen, "Design feedback on dashboard". General's most
   * recently active conversation, so it is what auto-select opens on
   * entering Chat - the showcase thread every rich message type in this
   * wave gets exercised on: spans two calendar dates (two dividers), both
   * directions, and one of each kind (plain text, an inline link, an
   * image with a caption, and a file message carrying two attachments at
   * once).
   */
  {
    id: 'm-11-1',
