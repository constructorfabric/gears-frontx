...
* `cpt-frontx-adr-template-manifest-contract` — Superseded by `cpt-frontx-adr-thin-template-manifest`; previously defined the template manifest publication contract declaring identity, version, ownership boundaries, and referenced templates.
...
* `cpt-frontx-adr-thin-template-manifest` — Narrows the template manifest to `name`, `version`, a required `description`, and `ownership.excludedSubtrees`, with `description` as the sole carrier of selection and usage semantics.
