...
* `cpt-frontx-adr-template-ownership-boundary-declaration` — Superseded by `cpt-frontx-adr-template-ownership-boundary-declaration`; previously defined the two-tier ownership-boundary declaration (exclusive subtrees plus shared-file region ownership with a declared merge).
* `cpt-frontx-adr-assembly-conflict-prevention` — Superseded by `cpt-frontx-adr-assembly-conflict-prevention`; previously detected and refused conflicting assembly via exact-path-equality comparison of declared boundaries plus a post-materialization boundary-honesty guard.
...
* `cpt-frontx-adr-project-upgrade-mechanism` — Superseded by `cpt-frontx-adr-project-upgrade-mechanism`; previously upgraded each applied template instance independently against a per-instance, provenance-derived baseline.
...
* `cpt-frontx-adr-assembly-conflict-prevention` — Detects ancestor/descendant containment, not only exact-path equality, as a conflict over canonicalized, fail-closed targets, while allowing deliberate nesting inside declared `excludedSubtrees`.
