...
* `cpt-frontx-adr-composed-template-resolution` — Superseded by `cpt-frontx-adr-composed-template-resolution`; previously assembled a repository by resolving a preset's manifest-declared referenced templates transitively in one operation.
* `cpt-frontx-adr-project-provenance-record` — Superseded by `cpt-frontx-adr-project-provenance-record`; previously recorded provenance as one record per applied template, with no single whole-repository origin.
...
* `cpt-frontx-adr-project-provenance-record` — Collapses template registration, applied-instance provenance, and project-owned exclusions into one Git-tracked `.frontx/project.json` document.
...
* `cpt-frontx-adr-composed-template-resolution` — Replaces manifest-declared preset composition and saved execution plans with an explicit target-keyed batch that `assemble` previews statelessly and `apply` re-validates and materializes.
...
* `cpt-frontx-adr-source-spec-syntax` — Registers a template under its manifest-declared name and pins a remote origin to the immutable value the resolver actually fetched, rather than the movable ref supplied.
