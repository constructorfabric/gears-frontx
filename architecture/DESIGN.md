...
| `cpt-frontx-nfr-evolvability` | Versioned releases without lockstep upgrades | The CLI's change-set & upgrade engine, owned by the [CLI's member DESIGN](../packages/cli/architecture/DESIGN.md) | The single authoritative change-set engine moves every target recorded under a template name atomically as one unit, computed against the baseline recorded for that name in the single project-state file `.frontx/project.json`, so each registered template in a repository adopts a newer version on its own cadence without a forced rewrite of targets not asked to move (`cpt-frontx-adr-atomic-all-targets-upgrade`, `cpt-frontx-adr-single-project-state-file`, `cpt-frontx-adr-cli-internal-decomposition`). The reviewed, approvable, non-destructive and reversible discipline this transition must ultimately satisfy is not a property this mechanism already delivers: per `cpt-frontx-adr-atomic-all-targets-upgrade` §More Information, it is a standing requirement carried forward onto a dedicated future changeset decision, and today `upgrade` moves a name's targets to a new version without an implemented review-and-approval step or a defined reversal path. | Blocked on that same future changeset decision (`cpt-frontx-adr-atomic-all-targets-upgrade` §More Information): once it lands, an end-to-end test can assert the applied change set equals the approved one, that a declined upgrade writes nothing, and that an applied upgrade is reversible. Verifiable today only: `upgrade` moves exactly the targets recorded under a template name, computed against the `project.json` baseline, and no other target. |
...
* `cpt-frontx-adr-source-spec-syntax` — Superseded by `cpt-frontx-adr-template-registration-and-origin-pinning`; previously defined the versioned source-spec syntax for template acquisition, including the optional subtree segment that lets one repository publish several addressable templates — its surviving holdings (subtree re-root and refusal rules, identity uniqueness, the `:`/`\`/control-character prohibition, and the `update-local` guard) are now carried forward explicitly by ADR-0040, with the manifest-identity portion by ADR-0035.
...
* `cpt-frontx-adr-template-manifest-contract` — Superseded by `cpt-frontx-adr-template-manifest-contract`; previously defined the template manifest publication contract declaring identity, version, ownership boundaries, and referenced templates.
* `cpt-frontx-adr-template-ownership-boundary-declaration` — Superseded by `cpt-frontx-adr-whole-target-ownership`; previously defined the two-tier ownership-boundary declaration (exclusive subtrees plus shared-file region ownership with a declared merge).
* `cpt-frontx-adr-assembly-conflict-prevention` — Superseded by `cpt-frontx-adr-nesting-aware-conflict-prevention`; previously detected and refused conflicting assembly via exact-path-equality comparison of declared boundaries plus a post-materialization boundary-honesty guard.
* `cpt-frontx-adr-composed-template-resolution` — Superseded by `cpt-frontx-adr-explicit-batch-application`; previously assembled a repository by resolving a preset's manifest-declared referenced templates transitively in one operation.
* `cpt-frontx-adr-project-provenance-record` — Superseded by `cpt-frontx-adr-single-project-state-file`; previously recorded provenance as one record per applied template, with no single whole-repository origin.
* `cpt-frontx-adr-project-upgrade-mechanism` — Superseded by `cpt-frontx-adr-atomic-all-targets-upgrade`; previously upgraded each applied template instance independently against a per-instance, provenance-derived baseline.
...
* `cpt-frontx-adr-template-manifest-contract` — Narrows the template manifest to `name`, `version`, a required `description`, and `ownership.excludedSubtrees`, with `description` as the sole carrier of selection and usage semantics.
* `cpt-frontx-adr-single-project-state-file` — Collapses template registration, applied-instance provenance, and project-owned exclusions into one Git-tracked `.frontx/project.json` document.
* `cpt-frontx-adr-whole-target-ownership` — Defines ownership as a template's whole target minus manifest-declared `excludedSubtrees` and project-declared `projectOwnedRoots`, arbitrating lifecycle rather than content.
* `cpt-frontx-adr-explicit-batch-application` — Replaces manifest-declared preset composition and saved execution plans with an explicit target-keyed batch that `assemble` previews statelessly and `apply` re-validates and materializes.
* `cpt-frontx-adr-nesting-aware-conflict-prevention` — Detects ancestor/descendant containment, not only exact-path equality, as a conflict over canonicalized, fail-closed targets, while allowing deliberate nesting inside declared `excludedSubtrees`.
* `cpt-frontx-adr-template-registration-and-origin-pinning` — Registers a template under its manifest-declared name and pins a remote origin to the immutable value the resolver actually fetched, rather than the movable ref supplied.
* `cpt-frontx-adr-atomic-all-targets-upgrade` — Upgrades every target recorded under a registered template name atomically, as one unit, using that name's own `{origin, version}` entry in `.frontx/project.json` as the baseline.
