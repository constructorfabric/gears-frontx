...
**Purpose**: Rename four identifiers (one with its folder) whose literal names describe a narrower or different scope than the thing they name actually has, consolidating the "pending follow-up" notes each currently carries in its own isolated location into one tracked work item, per the precedent §2.22 sets for tracking a rename wave rather than scattering follow-up prose across unrelated sections.
