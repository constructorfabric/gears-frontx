...
- [x] `p1` - **ID**: `cpt-frontx-flow-cli-scaffolding-scaffold-project`
...
**Steps**:
1. [x] - `p1` - Developer invokes the project-level namespace scaffold command with a template reference and a target directory path. - `inst-invoke-project-cmd`
2. [x] - `p1` - **IF** the template reference resolves to no entry in the local template inventory - `inst-check-template-resolved`
   1. [x] - `p1` - **RETURN** scaffold aborted — template reference not found in local inventory. - `inst-abort-not-found`
3. [x] - `p1` - The CLI dispatches the command to the one shared resolver with the template reference and kind `project`. - `inst-dispatch-to-resolver`
4. [x] - `p1` - The shared resolver locates the installed template entry for the given reference. - `inst-resolver-locate`
5. [x] - `p1` - **IF** the target directory already exists and contains conflicting content - `inst-check-target-conflict`
   1. [x] - `p1` - **RETURN** scaffold aborted — target directory conflict detected. - `inst-abort-conflict`
6. [x] - `p1` - The CLI applies the project scaffolding process to the resolved template entry and the target directory. - `inst-apply-project-scaffold`
7. [x] - `p1` - **RETURN** scaffold complete — project written to the target directory. - `inst-return-done`

### Scaffold Microfrontend

- [x] `p1` - **ID**: `cpt-frontx-flow-cli-scaffolding-scaffold-mfe`
...
**Steps**:
1. [x] - `p1` - Developer invokes the microfrontend-level namespace scaffold command with a template reference and a target directory path. - `inst-invoke-mfe-cmd`
2. [x] - `p1` - **IF** the template reference resolves to no entry in the local template inventory - `inst-check-template-resolved`
   1. [x] - `p1` - **RETURN** scaffold aborted — template reference not found in local inventory. - `inst-abort-not-found`
3. [x] - `p1` - The CLI dispatches the command to the one shared resolver with the template reference and kind `microfrontend`. - `inst-dispatch-to-resolver`
4. [x] - `p1` - The shared resolver locates the installed template entry for the given reference. - `inst-resolver-locate`
5. [x] - `p1` - **IF** the target directory already exists and contains conflicting content - `inst-check-target-conflict`
   1. [x] - `p1` - **RETURN** scaffold aborted — target directory conflict detected. - `inst-abort-conflict`
6. [x] - `p1` - The CLI applies the microfrontend scaffolding process to the resolved template entry and the target directory. - `inst-apply-mfe-scaffold`
7. [x] - `p1` - **RETURN** scaffold complete — microfrontend written to the target directory. - `inst-return-done`
...
- [x] `p1` - **ID**: `cpt-frontx-algo-cli-scaffolding-namespace-routing`
...
**Steps**:
1. [x] - `p1` - Receive the command invocation and extract its namespace label, command name, and arguments. - `inst-receive-invocation`
2. [x] - `p1` - **MATCH** namespace label - `inst-match-namespace`
   - [x] - `p1` - **CASE** `project`: associate the command with the project-level namespace. - `inst-case-project`
   - [x] - `p1` - **CASE** `microfrontend`: associate the command with the microfrontend-level namespace. - `inst-case-mfe`
   - [x] - `p1` - **DEFAULT**: **RETURN** command routing failed — unrecognized namespace label. - `inst-default-unknown`
3. [x] - `p1` - Verify the command name is registered within the resolved namespace. - `inst-verify-command`
4. [x] - `p1` - **IF** the command name is not registered in the namespace - `inst-check-registered`
   1. [x] - `p1` - **RETURN** command routing failed — command not found in namespace. - `inst-abort-not-found`
5. [x] - `p1` - Forward the command arguments to the one shared resolver together with the resolved namespace and command name. - `inst-forward-to-shared-resolver`
6. [x] - `p1` - **RETURN** the result produced by the shared resolver. - `inst-return-result`

### Project Scaffolding

- [x] `p1` - **ID**: `cpt-frontx-algo-cli-scaffolding-project-scaffold`
...
**Steps**:
1. [x] - `p1` - Receive the resolved template entry and the target directory path. - `inst-receive-entry`
2. [x] - `p1` - Read the template manifest from the resolved template entry. - `inst-read-manifest`
3. [x] - `p1` - Validate that the template manifest declares kind `project`. - `inst-validate-kind`
4. [x] - `p1` - **IF** the manifest declared kind is not `project` - `inst-check-kind`
   1. [x] - `p1` - **RETURN** scaffold aborted — template kind mismatch for project scaffolding. - `inst-abort-kind-mismatch`
5. [x] - `p1` - Enumerate the content items declared in the template manifest. - `inst-enumerate-items`
6. [x] - `p1` - **FOR EACH** content item in the template - `inst-foreach-item`
   - [x] - `p1` - Resolve the item's destination path relative to the target directory. - `inst-resolve-dest`
   - [x] - `p1` - Write the item to its destination path. - `inst-write-item`
7. [x] - `p1` - **RETURN** scaffold complete — all project content items written to the target directory. - `inst-return-complete`

### Microfrontend Scaffolding

- [x] `p1` - **ID**: `cpt-frontx-algo-cli-scaffolding-mfe-scaffold`
...
1. [x] - `p1` - Receive the resolved template entry and the target directory path. - `inst-receive-entry`
2. [x] - `p1` - Read the template manifest from the resolved template entry. - `inst-read-manifest`
3. [x] - `p1` - Validate that the template manifest declares kind `microfrontend`. - `inst-validate-kind`
4. [x] - `p1` - **IF** the manifest declared kind is not `microfrontend` - `inst-check-kind`
   1. [x] - `p1` - **RETURN** scaffold aborted — template kind mismatch for microfrontend scaffolding. - `inst-abort-kind-mismatch`
5. [x] - `p1` - Enumerate the content items declared in the template manifest. - `inst-enumerate-items`
6. [x] - `p1` - **FOR EACH** content item in the template - `inst-foreach-item`
   - [x] - `p1` - Resolve the item's destination path relative to the target directory. - `inst-resolve-dest`
   - [x] - `p1` - Write the item to its destination path. - `inst-write-item`
7. [x] - `p1` - **RETURN** scaffold complete — all microfrontend content items written to the target directory. - `inst-return-complete`

## 4. States (CDSL)

### Scaffold Operation State Machine

- [x] `p2` - **ID**: `cpt-frontx-state-cli-scaffolding-scaffold-op`
...
**Transitions**:
1. [x] - `p1` - **FROM** REQUESTED **TO** RESOLVED **WHEN** the shared resolver successfully locates the installed template entry for the given reference. - `inst-transition-req-resolved`
2. [x] - `p1` - **FROM** REQUESTED **TO** ABORTED **WHEN** the template reference cannot be resolved from the local template inventory. - `inst-transition-req-aborted-unresolved`
3. [x] - `p1` - **FROM** RESOLVED **TO** SCAFFOLDED **WHEN** the scaffolding process writes all template content items to the target directory without conflict. - `inst-transition-resolved-scaffolded`
4. [x] - `p1` - **FROM** RESOLVED **TO** ABORTED **WHEN** a target directory conflict or a template kind mismatch is detected during scaffolding. - `inst-transition-resolved-aborted`

## 5. Definitions of Done

### Two-Namespace Command Surface

- [x] `p1` - **ID**: `cpt-frontx-dod-cli-scaffolding-namespace-surface`
...
- [x] `p1` - **ID**: `cpt-frontx-dod-cli-scaffolding-project-scaffold`
...
- [x] `p1` - **ID**: `cpt-frontx-dod-cli-scaffolding-mfe-scaffold`
