...
Chosen option: **abstract registry facade with factory injection**, because it is the only option that gives consumers a stable contract (`MfeRegistry`) independent of internal coordination machinery, keeps the concrete implementation substitutable behind that contract, and provides a single controlled construction point at which the type-system provider is injected. The abstract `MfeRegistry` (`packages/mfes/src/registry/MfeRegistry.ts`) declares the runtime's public operations — domain and extension registration, actions-chain acceptance, query, and the read-only `typeSystem` provider — and instances are obtained through the factory's `build({ typeSystem })` (`packages/mfes/src/runtime/DefaultMfeRegistryFactory.ts`). The concrete factory and registry implementation are internal and are not part of the package's public surface.

The facade's actions-chain operation exposes acceptance only: it yields nothing a consumer can await for the chain's own execution and takes no per-call execution-options argument; dispatch, refusal, and settlement semantics are decided in `cpt-frontx-adr-action-dispatch-and-chaining`. An operation that instead reported when a chain finished would expose the mediator's internal settlement through the facade — the exact internal coordination machinery the encapsulation driver above rules out of the public surface — so acceptance-only is this driver applied to this operation, not an additional rule layered onto it. The result type that would carry a completion report, and the per-call options type that would let a caller influence execution once accepted, accordingly leave the public surface entirely: both exist only to report or shape what the executor behind the facade does, and a facade that exposes either is no longer only the narrow contract this decision chose.
...
* Good, because keeping the actions-chain operation acceptance-only means a change to how the mediator settles a chain internally never forces a change to the facade, which is the narrow-surface driver holding under the one operation most tempted to leak internal coordination outward.
* Bad, because a consumer that needs to know a chain's outcome cannot obtain it from the facade and must express that need as a node inside the chain itself, rather than by awaiting the dispatch call.
...
Architecture review confirms that registry construction crosses the package's public surface only as the abstract `MfeRegistry` contract and the `createMfeRegistryFactory` creation function, and that the concrete implementation and factory remain internal. Architecture review further confirms that `MfeRegistry`'s actions-chain operation returns nothing a caller can await for the chain's own execution and takes no per-call execution-options argument, and that neither a completion-bearing result type nor a per-call execution-options type is exported from the package's barrel. A continuous-integration check (`scripts/mfes-import-boundary-check.mjs`, wired as `npm run policy:mfes-import-boundary` in `.github/workflows/main.yml`) confirms both halves over this repository: the barrel exports no concrete-implementation name (the `Default*` naming rule), and no consumer in this repository imports a concrete class or deep-imports past the barrel — every instance is obtained through `createMfeRegistryFactory().build({ typeSystem })`.

The two halves do not reach equally far. The **barrel half** is structural and answers for every consumer anywhere, because a name the barrel does not export cannot be imported from any repository; that half is whole here. The **consumer half** reaches only as far as the tree it walks, so it answers for this repository's consumers and for no others: a repository that holds consumers of its own confirms that half by running the same rule over its own tree, and a consumer in a repository that runs no such check is unchecked. This is why the barrel half is the one that must stay complete. The reach of each half is recorded with the boundary it guards, in `packages/mfes/architecture/DESIGN.md` (§3.3).
...
* `cpt-frontx-constraint-mfes-origin-invariant-chain-execution` — the facade's actions-chain operation carries this constraint's surface consequence: acceptance-only, with no completion-bearing result type or per-call execution-options type on the public surface.
