...
**URL projection of domain occupancy (present detail, non-binding).** Extension-domain occupancy has a URL projection, and this release's own projection rule carries at most one entry of a domain's own key at a time — zero-or-one, never several at once — regardless of which strategy governs the domain. A domain governed by the exclusive strategy, always holding exactly one occupant with no empty state to represent, is the natural fit for a zone's own axial domain: its occupant's own declared segment continues the address bar's own pathname, making the pathname an observable reflection of which occupant currently holds the domain. The same exclusive strategy may instead be projected as a parallel axis — legal, and still zero-or-one, with a new occupant's pre-emptive replacement of the value the key already holds mirroring the strategy's own pre-emptive replacement with no explicit unmount — but the axial projection is preferred whenever a domain holds a zone's own sole occupancy, since continuing the pathname needs no dedicated key of its own. A domain governed by the optional strategy may be axial the same way when a zone's own author chooses it as that zone's one axial domain — the pathname simply ends where the domain currently holds no occupant — or may instead be projected as a parallel axis: an unrepeated, optional entry present while the domain holds its one occupant and absent while it holds none. A domain governed by the concurrent strategy — several occupants held side by side at once — is not projected by this decision's own projection rule in this release: expressing more than one occupant of the same key in the URL, including what a multiple-occupant transition and a per-entry identity would need to look like, is deferred until a first real consumer of a concurrently-occupied, projected domain exists; that consumer's own requirements will shape the form this projection eventually takes. Until then, a domain governed by the concurrent strategy that is not projected simply remains runtime state the routing package takes no part in — occupancy that never reaches the URL is outside this projection rule's own scope, not a gap in it. Only one domain within a zone is ever axial; every other domain that zone projects into the URL at all is necessarily a parallel axis, regardless of which of these strategies it runs. Either way, a navigation act is an entry into the same mount mechanism this decision governs, not a second occupancy mechanism running alongside it. The URL is the source of truth for that reflection, and occupancy is its derivative; the reverse mapping — recovering the URL from an occupancy change — applies only when a mount is triggered by something other than navigation. The routing package publishes that reflection as a signal, not as a guarantee it keeps itself: its Route Ownership Signal feature ([packages/routing/architecture/features/route-ownership-signal/FEATURE.md](../../packages/routing/architecture/features/route-ownership-signal/FEATURE.md)) exposes the owner-resolution primitive, an observable owner-change signal, and a URL back-projection helper for either projection, but the binding mechanics — actually mounting or unmounting an occupant in response to that signal, and resolving a race between two mounts — are performed by the host's own glue on top of this decision's mount strategies and registry, not by the routing package. This decision continues to own the mount strategies and the cardinality matrix.
