...
**URL projection of domain occupancy (present detail, non-binding).** Extension-domain occupancy has a URL projection, and that projection places every one of this decision's own strategies. A domain governed by the exclusive strategy, always holding exactly one occupant with no empty state to represent, is the natural fit for a zone's own axial domain: its occupant's own declared segment continues the address bar's own pathname, making the pathname an observable reflection of which occupant currently holds the domain. A domain governed by the optional strategy may be axial the same way when a zone's own author chooses it as that zone's one axial domain — the pathname simply ends where the domain currently holds no occupant — or, like a domain governed by the concurrent strategy, may instead be projected as a parallel axis: a dedicated query-string key whose entry multiplicity follows the strategy — an unrepeated, optional entry for the optional strategy's own zero-or-one occupancy, repeated entries for the concurrent strategy's own several-at-once occupancy. Only one domain within a zone is ever axial; every other domain that zone projects into the URL at all is necessarily a parallel axis, regardless of which of these three strategies it runs. Either way, a navigation act is an entry into the same mount mechanism this decision governs, not a second occupancy mechanism running alongside it. The URL is the source of truth for that reflection, and occupancy is its derivative; the reverse mapping — recovering the URL from an occupancy change — applies only when a mount is triggered by something other than navigation. The routing package publishes that reflection as a signal, not as a guarantee it keeps itself: its Route Ownership Signal feature ([packages/routing/architecture/features/route-ownership-signal/FEATURE.md](../../packages/routing/architecture/features/route-ownership-signal/FEATURE.md)) exposes the owner-resolution primitive, an observable owner-change signal, and a URL back-projection helper for either projection, but the binding mechanics — actually mounting or unmounting an occupant in response to that signal, and resolving a race between two mounts — are performed by the host's own glue on top of this decision's mount strategies and registry, not by the routing package. This decision continues to own the mount strategies and the cardinality matrix.
