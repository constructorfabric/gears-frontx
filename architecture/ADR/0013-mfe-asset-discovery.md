...
* Completeness for an isolated load — the description must carry everything an isolated load needs: the base location for assets, the file backing each exposed entry, the entry's stylesheet assets, and the full set of shared-dependency declarations in a stable **enumeration order** — so a load needs no out-of-band lookup. That enumeration order fixes only the sequence in which a consumer visits the declarations; it is neither a fetch order nor a dependency order. Resolving the declarations leaves-first is the consumer's own pass over the sources it fetches, which is where dependency-order correctness and circular-dependency handling are decided (`cpt-frontx-adr-mfe-load-isolation`).
...
* **Enriched-manifest-driven discovery** — the runtime locates and loads a microfrontend from a published, enriched manifest that carries the asset base location, per-entry backing file and stylesheet assets, and the shared-dependency declarations in the enumeration order the completeness driver fixes; the manifest is the single declared contract for discovery.
...
A continuous-integration check confirms manifest conformance and the discovery path. A schema/contract check confirms that a microfrontend's published manifest carries the required locating fields — asset base location, per-entry backing file and stylesheet assets, and the shared-dependency declarations in the enumeration order the completeness driver fixes — and rejects a manifest missing them. An automated runtime test confirms that a load resolves its asset base location and per-entry backing file from the manifest alone and completes without fetching or parsing a compiled remote-entry module. The grounding mechanisms in the present concrete instantiation are the manifest type `MfManifest` (`packages/mfes/src/manifest/mf-manifest.ts`) with its `metaData.publicPath` asset base (`packages/mfes/src/manifest/mf-manifest.ts`) and its `shared[]` declarations in that enumeration order (`packages/mfes/src/manifest/mf-manifest.ts`), the per-entry `exposeAssets` asset record carried on the entry, and the runtime load path in `MfeHandlerMF` that derives the base location from `manifest.metaData.publicPath` (`packages/mfes/src/handler/mfe-handler-mf/MfeHandlerMF.ts`) and the backing file from `exposeAssets.js.sync[0]` (`packages/mfes/src/handler/mfe-handler-mf/MfeHandlerMF.ts`). The handler carries no remote-entry parsing on or beside the load path.

## Pros and Cons of the Options

### Enriched-manifest-driven discovery

The runtime locates and loads a microfrontend from a published, enriched manifest carrying the asset base location, per-entry backing file and stylesheet assets, and the shared-dependency declarations in the enumeration order the completeness driver fixes.
