...
* Bad, because a manifest published under the retired ADR 0018 shape is refused outright by every command that reads a manifest, including `install` — there is no migration path, lossless or otherwise, from the retired shape into the current one. This revokes the guarantee ADR 0018 made when it left `description` optional specifically "so that manifests published before it was declared stay conforming and installable": a legacy manifest is no longer installable at all until its author converts it to the current four-field shape by hand, regardless of whether it carries a description or how narrowly its retired `exclusiveSubtrees` claimed. With this repository's own two templates as the only currently known consumers of the retired shape, and their conversion tracked separately (`cpt-frontx-feature-template-territory-conversion`), this is judged an acceptable YAGNI cost rather than cause to build and maintain a generic translation path for a third-party legacy template this repository does not have.
...
* **REL** — Not applicable, because there is no service availability target for a local manifest read; the outright refusal of a malformed or legacy manifest is covered under Confirmation.
...
* **MAINT** — addressed: fewer declared categories and a single semantic carrier concentrate what an author and a reviewer must maintain, and refusing a legacy manifest outright keeps the retired shape from leaking into ongoing maintenance of the current type, with no migration path of its own to maintain.
