...
A continuous-integration check confirms the ABI contract holds on both sides of the boundary. On the runtime side, an automated test confirms that a lazy reference exercised within a load resolves into that load's existing graph and reuses its shared-dependency bindings (a singleton dependency observed identical across the eager and lazy chunks of one load), and that a distinct concurrent load resolves its own lazy references through its own resolver rather than a sibling's. On the consumer-contract side, a build-time check rejects any lazy reference whose argument is not statically resolvable, with a diagnostic pointing at the offending reference. The grounding mechanisms in the present concrete instantiation are, on the build side, the AST rewrite carried by the `frontxMfGts` plugin — `LazyImportTransformer` and `transformLazyImports`, run at the `renderChunk` stage, which rewrite each statically resolvable dynamic import to the `__frontx_lazy` call and raise a build-time error on a non-statically-resolvable argument; that half lives in template territory and is therefore cited by construct rather than as authoritative grounding, per `cpt-frontx-adr-template-territory-traceability`. On the runtime side the grounding is the per-load resolution path: the per-load loader stub minted by `ensureLazyLoaderUrl` in `MfeHandlerMF` (`packages/mfes/src/handler/mfe-handler-mf/MfeHandlerMF.ts`), the host-side per-load resolver table `LazyLoaderRegistry` (`packages/mfes/src/lazy-loader/lazy-loader-registry.ts`), and `resolveLazyChunk`, which funnels a lazy reference back into the same load's graph and shared-dependency bindings (`packages/mfes/src/handler/mfe-handler-mf/MfeHandlerMF.ts`).
