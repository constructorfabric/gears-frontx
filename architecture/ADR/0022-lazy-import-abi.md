...
- [Pros and Cons of the Options](#pros-and-cons-of-the-options)
  - [Plugin AST transform of dynamic `import()` + handler runtime resolver (`__hai3_lazy`)](#plugin-ast-transform-of-dynamic-import--handler-runtime-resolver-hai3lazy)
...
* Plugin AST transform of dynamic `import()` + handler runtime resolver (`__hai3_lazy`).
...
Chosen option: **"Plugin AST transform of dynamic `import()` + handler runtime resolver (`__hai3_lazy`)"**.

The `frontx-mf-gts` Vite plugin uses Vite's `transform` hook (a stable public API operating on the module AST, not on compiled output) to rewrite every dynamic `import('<relative-path>')` call in vendor MFE source to `__hai3_lazy('<relative-path>')`. `MfeHandlerMF` provides a runtime function `__hai3_lazy.load(relativePath)` bound into the per-load scope that resolves the relative path against the manifest's `publicPath`, fetches the lazy chunk's source text through the existing URL-keyed `sourceTextCache`, recursively rewrites bare specifiers AND nested `__hai3_lazy()` calls against the parent load's `sharedDepBlobUrls`, creates a per-load blob URL within the parent load's `LoadBlobState.blobUrlMap`, and returns the blob URL string — the caller's transformed code resolves that string via `import()` and gets a `Promise<Module>` with the parent load's shared modules already wired.
...
* Good, because lazy semantics are preserved — `__hai3_lazy.load(path)` returns a blob URL, and the caller's transformed `import(blobUrl)` produces a `Promise<Module>` exactly like the original `import(relativePath)` would have; chunks are fetched on demand, not eagerly.
...
* Bad, because the handler's runtime now owns a small public-shape ABI (`__hai3_lazy`) that vendor MFEs depend on through their compiled output. Mitigation: the ABI is host-injected at evaluation time, not part of any vendor-import-able package; changes to the resolver's internals are invisible to the vendor as long as the call shape (`__hai3_lazy('<path>')` returns a `Promise<blobUrlString>` or equivalent) stays stable.
...
* Bad, because two systems (plugin + handler) must stay in sync on the ABI shape — `__hai3_lazy` is a contract surface that both sides depend on.
...
* Good, because the entry chunk becomes a single self-contained bundle — no lazy resolution needed at runtime, no `__hai3_lazy` ABI.
