...
* Neutral, because independent member versioning is expressed in the ref, the shape offering no second place to carry it: the version selector is repository-scoped while the subtree segment is template-scoped, so `@ref` names one point in the repository's history and every member addressed from that point is addressed at it. A family released as one set gets exactly what it needs from this and pays nothing. A family whose members version independently gets the same guarantee only if the refs it publishes are themselves per-member - a ref namespace in which a member's release names that member - because a repository-wide ref advanced for one member moves the address of every other member with it. Which ref namespace a family publishes for that is the family's own convention and is not fixed here.
...
* Bad, because every surface that carries a reference must be widened to carry the extra values - provenance records, a preset's declared member references, and command arguments.
...
* Bad, because a composed preset's member templates end up in separate repositories that must be kept version-aligned by convention rather than by construction.
...
* `cpt-frontx-fr-cli-composed-template-resolution` - A preset's declared member references are the motivating case for addressing templates that share a repository, and they carry the shape this decision fixes.
