...
* Neutral, because independent sibling versioning is expressed in the ref, the shape offering no second place to carry it: the version selector is repository-scoped while the subtree segment is template-scoped, so `@ref` names one point in the repository's history and every sibling addressed from that point is addressed at it. A family released as one set gets exactly what it needs from this and pays nothing. A family whose siblings version independently gets the same guarantee only if the refs it publishes are themselves per-sibling - a ref namespace in which a sibling's release names that sibling - because a repository-wide ref advanced for one sibling moves the address of every other sibling with it. Which ref namespace a family publishes for that is the family's own convention and is not fixed here.
...
Amended 2026-09-17 (#609): the consequence that independent sibling versioning is carried in the ref, the shape offering no second place for it.
