...
The host-prefixed reference shape this decision reuses is already established in the platform substrate: external sources are expressed in the host-prefixed `host:owner/repo` form in `.cypilot/config/core.toml` (for example the `source = "github:cyberfabric/cyber-pilot-kit-sdlc"` entry under `[kits.sdlc]`). That substrate entry is cited only as a neutral, timeless example of the established shape so this decision's reuse is accurate; the concrete `github:` token and that specific repository are non-binding present detail, not part of this decision's durable identity. The resolver that consumes a source-spec is decided in `cpt-frontx-adr-template-externalization-resolution`; the source registry actor is `cpt-frontx-actor-github`.
