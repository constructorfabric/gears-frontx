...
- LSP: PASS. The abstract `MountStrategy` makes `mount` abstract and `unmount` optional; every concrete strategy satisfies the abstract base contract on the abstract surface. The strict cardinality matrix (`cpt-frontx-algo-mfe-registry-cross-validate-handlers`) enforces cardinality declaration at registration time; LSP-substitutability on the abstract surface is preserved.
...
- `cpt-frontx-feature-mfe-registry` — owns the algorithm and public-API surface.
