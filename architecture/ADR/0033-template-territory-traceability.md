...
Compliance is confirmed by the artifact registry and by review. The registry admits only the ecosystem's own source paths to traceability scanning, so no template file is scanned for markers and no marker there is validated; `cfs validate` passing therefore never depends on template payload. Since the template subtrees left this repository (`cpt-frontx-adr-template-repository-separation`), that holds by absence here, and the registry's exclusion of those paths retires with the directories it named. Review confirms that no FEATURE in this tree names a path under a template subtree, that a change adding a `@cpt-` marker to template territory is refused, and that a change modifying an already-marked template file removes every residue marker that file carries while keeping the mechanical marker deletion easy to distinguish from substantive change and documenting any consumer-visible behaviour at code altitude in the same change.
...
**Deferred deliberately, and since resolved in part**: whether template territory gets an artifact tree of its own was **not decided here**. It was deferred until template ownership had settled. `cpt-frontx-adr-template-repository-separation` settled the ownership half: template territory now lives in its own repository, `constructorfabric/gears-frontx-templates`. Whether that repository authors an artifact tree for its templates is its own decision to make under its own governance, and is no longer a question this repository holds open. This decision continues to state what it always stated — that the ecosystem artifact tree does not specify template internals, and that `@cpt-` markers found in template payload bind nothing.
