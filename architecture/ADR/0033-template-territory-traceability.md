...
* `cpt-frontx-adr-template-classification` — Templates are payload the tooling resolves, applies, and upgrades uniformly without branching on type; this decision is the artifact-side consequence: the ecosystem specifies the mechanism, not any template's internals.
