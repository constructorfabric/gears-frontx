...
* **Declared referenced templates (presets)** — a template may reference other templates to be applied together as a set (a preset), so the manifest must declare those references for resolution to read (a requirement later retired and superseded by `cpt-frontx-adr-explicit-batch-application`).
