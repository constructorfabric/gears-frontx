...
- **O4 - i18n-namespace registration timing and retention.** A Screen Template Developer whose sibling renders a shell-facing menu label **MUST** dispatch the namespace-and-per-locale-dictionary registration action by the time that sibling's own extension is admitted into the screen extension domain - not only when that sibling's own screen content becomes the routed one - because the domain admits every currently-applied sibling's extension at once (§3.6), and every applied sibling's own label must be registrable before any of them is the routed screen. The shell **MUST** retain a sibling's registration for as long as that sibling's extension stays admitted, independent of which sibling's screen content is currently routed; a sibling's label going missing because a different sibling is currently routed is a defect in this contract, not an accepted gap. A sibling that never dispatches the action renders with no shell-resolved label, because the shell holds none of the sibling's translations to fall back to.
